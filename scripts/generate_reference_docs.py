...
def render_artifacts() -> str:
    ...
    lines = [
        ...
        f"  <arch>/{TG_SUBDIR}/               TG init.yaml, plan.md, worklog.md, cases",
        f"  <arch>/{CE_SUBDIR}/plan/          CE named plans ({{slug}}_plan.md)",
        f"  <arch>/session_handoff.md         /handoff conversation summary",
        f"  <arch>/{STATE_SUBDIR}/            Pilot state and leases",
        f"  <arch>/{RUNS_SUBDIR}/             action bundles, staging and receipts",
        f"  <arch>/{CONTEXT_SUBDIR}/          pilot_params and run projections",
