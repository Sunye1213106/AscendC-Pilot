...
def test_the_operator_contract_holds_over_the_synthetic_sweep(fag):
    import importlib.util
    import sys
    from pathlib import Path

    path = Path(__file__).resolve().parents[2] / "_probe_contract_audit.py"
    spec = importlib.util.spec_from_file_location("_probe_contract_audit", path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)

    report = CA.audit_many(module.synthetic_cases(), fag)
