...
Execution contract:
1. Goal Contract + `goal-intake` (`auto`) own orchestration. Spec owns phase and lease. Prefer Host `pilot_run` to start `uo-init` when it is not already the active workflow; do not call domain CLIs directly.
...
Execution contract:
1. Goal Contract + `goal-intake` (`auto`) own orchestration. Spec owns phase and lease. Prefer Host `pilot_run` to start `{workflow_id}` when it is not already the active workflow; do not call domain CLIs directly.
