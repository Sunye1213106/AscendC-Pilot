...
def test_candidate_paths_excludes_tests_examples_by_default() -> None:
    from uo.scripts.macro_scope_scan import _candidate_paths

    all_files = [
        "DemoOp/op_host/a.cpp",
        "DemoOp/op_host/arch35/t.cpp",
        "DemoOp/op_host/arch35/t.h",
        "DemoOp/tests/ut/op_host/test_a.cpp",
        "DemoOp/examples/demo.cpp",
        "DemoOp/op_kernel/k.cpp",
    ]
    matched = _candidate_paths(Path("."), "DemoOp", all_files, [], op_rel_prefix="DemoOp")
    assert "DemoOp/op_host/a.cpp" in matched
    assert "DemoOp/op_host/arch35/t.cpp" in matched
    assert "DemoOp/op_host/arch35/t.h" in matched
    assert "DemoOp/op_kernel/k.cpp" in matched
    assert "DemoOp/tests/ut/op_host/test_a.cpp" not in matched
    assert "DemoOp/examples/demo.cpp" not in matched


def test_candidate_paths_seed_can_force_include_excluded() -> None:
    from uo.scripts.macro_scope_scan import _candidate_paths
    ...
    matched = _candidate_paths(
        Path("."),
        "DemoOp",
        all_files,
        ["DemoOp/tests/ut/op_host/test_a.cpp"],
        op_rel_prefix="DemoOp",
    )
    assert "DemoOp/tests/ut/op_host/test_a.cpp" in matched


def test_build_summary_counts_layers_cpp_and_h() -> None:
    from uo.scripts.macro_scope_scan import _build_summary_counts

    candidates = [
        "DemoOp/op_host/def.cpp",
        "DemoOp/op_host/tiling.cpp",
        "DemoOp/op_host/infershape.cpp",
        "DemoOp/op_host/arch35/common.cpp",
        "DemoOp/op_host/arch35/common.h",
        "DemoOp/op_host/arch35/normal.cpp",
        "DemoOp/op_host/arch35/normal.h",
        "DemoOp/op_host/arch35/varlen.cpp",
        "DemoOp/op_kernel/k.cpp",
        "DemoOp/op_kernel/arch35/a.h",
        "common/op_kernel/arch35/x.h",
    ]
    summary = _build_summary_counts(candidates, operator_path="DemoOp", architecture="arch35")
    by_layer = {row["layer"]: row for row in summary["included_layers"]}
    assert by_layer["op_host/ (top-level)"]["cpp"] == 3
    assert by_layer["op_host/ (top-level)"]["h"] == 0
    assert by_layer["op_host/arch35/"]["cpp"] == 3
    assert by_layer["op_host/arch35/"]["h"] == 2
    assert by_layer["op_kernel/ (top-level)"]["cpp"] == 1
    assert by_layer["op_kernel/arch35/"]["h"] == 1
    assert summary["included_totals"]["operator"]["total"] == 10
    assert summary["included_totals"]["common"]["h"] == 1
    assert any("tests" in str(c.get("category", "")) for c in summary["excluded_categories"])
...
def test_strip_cbm_stage_path() -> None:
    ...
    raw = (
        ".ascendc-pilot/uo/cbm/index_stage/Demo/"
