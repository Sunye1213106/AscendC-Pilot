...
from uo_init.source_layout import ARCH_DIR_RE, UNIFIED_ARCH_DIR, arch_number
...
def _pick_arch(op: Path) -> str:
    """Newest ``arch*`` implementation folder, or ``default`` when none exist.

    Official operator trees split implementations under arch22/arch35/….
    A tree without those folders is one implementation — do not invent arch35.
    """
    archs = _list_archs(op)
    if archs:
        return max(archs, key=_arch_sort_key)
    return UNIFIED_ARCH_DIR
...
def main() -> int:
    ...
    doc: dict[str, Any] = {
        ...
        "date": time.strftime("%Y-%m-%d"),
        "profile": {
            "UO_KERNEL_MAX_VARIANTS": os.environ.get("UO_KERNEL_MAX_VARIANTS") or "1",
            "UO_WITH_KERNEL": os.environ.get("UO_WITH_KERNEL") or "1",
