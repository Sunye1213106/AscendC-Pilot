"""Tests for UO scoring + Harness disambiguation loop (十二条)."""
...
def test_harness_engines_registered() -> None:
    import sys
    from pathlib import Path
    ...
    if harness_root.is_dir() and str(harness_root) not in sys.path:
        ...
        inserted = True
    try:
        from ascendc_harness.actions.engines import ENGINE_REGISTRY
        ...
        # Avoid leaving harness on sys.path — it enables reject_key_patch_batch
        ...
        for mod in list(sys.modules):
            if mod == "ascendc_harness" or mod.startswith("ascendc_harness."):
                del sys.modules[mod]
