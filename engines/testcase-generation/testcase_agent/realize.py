            ...
            blocked.append(
                {
                    ...
                    "status": "realize_blocked",
                    "reason": (
                        "CONTRACT_REQUIRED: provide consumer_schema/realization_map from acp contract_build"
                    ),
                    "model": model,
                }
            )
...
def match_realization(model: dict[str, Any], candidate: dict[str, Any], input_realization: dict[str, Any]) -> dict[str, Any]:
    ...
    if not input_realization:
        return {
            "status": "blocked",
            "reason": "no input_realization catalog; use acp contract_build consumer_schema emit",
            "refs": [],
        }
...
def build_case_row(candidate: dict[str, Any], model: dict[str, Any], realization: dict[str, Any], idx: int) -> dict[str, Any]:
    ...
    raise RuntimeError(
        "CSV_CONTRACT_REQUIRED: run acp run-action contract_build and use "
