...
def validate_intake(export_payload: dict[str, Any], final_validation: dict[str, Any]) -> ValidationReport:
    ...
    if files.get("contracts/testcase.yaml"):
        report.add(
            ...
            "error",
            "UO contracts/testcase.yaml is not a valid TG input; run acp run-action contract_build",
    ...
    # Hard-ref checks apply after acp contract_build writes the TG contract.
    del known_ids
