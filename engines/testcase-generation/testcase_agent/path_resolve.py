...
def resolve_operator_project_root(path: Path) -> Path:
    ...
    if root.name in {".ascendc-pilot", ".ascendc-agent", ".understand-operator"}:
        return root.parent
    if root.exists() and (
        (root / ".ascendc-pilot").is_dir()
        or (root / ".ascendc-agent").is_dir()
        or (root / ".understand-operator").is_dir()
    ):
        return root
    if not root.exists():
...
def normalize_contract_root(path: Path) -> Path:
    """Accept realization/, .ascendc-pilot/tg/, or a dir that contains realization/."""
...
def install_contract_into_project(project_root: Path, op_name: str, contract_root: Path) -> Path:
    """Copy/reuse contract artifacts into <project>/.ascendc-pilot/tg/realization/."""
