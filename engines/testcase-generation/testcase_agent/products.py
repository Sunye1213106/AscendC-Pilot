    ...
    *,
    ...
    if not cov:
        ...
        if l2_is_full_cross(cov):
            # Full crossing is generated by the engine. The plan's contribution is
            # the exclusion set: which cells are already known to conflict. An empty
            # exclusion set means the plan handed Solve a raw cartesian product.
            exclusions = l2_block.get("exclusions")
            if not isinstance(exclusions, list) or not exclusions:
                errors.append(
                    "coverage.L2 full_cross requires a non-empty exclusions list; "
                    "a raw cartesian product means no reachability analysis was done"
                )
            else:
