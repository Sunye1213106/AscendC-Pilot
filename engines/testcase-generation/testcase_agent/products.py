...
_EVIDENCE_KINDS = frozenset({"replay_field", "derived", "dispatch_map", "probe"})
    ...
    *,
    ...
    for idx, row in enumerate(targets):
        ...
        if kind not in _EVIDENCE_KINDS:
            errors.append(f"{tid}: evidence.kind must be replay_field|derived|dispatch_map|probe")
    ...
    for idx, row in enumerate(fence.get("dimensions") or []):
        ...
        reqs = [str(x).strip() for x in (classifier.get("requires") or []) if str(x).strip()]
        probe_only = bool(reqs) and all(r.startswith(("probe.", "replay.")) for r in reqs)
        if not controls and not probe_only:
    ...
    if not cov:
        ...
        l2_block = cov.get("L2")
        from testcase_agent.coverage.compile import l2_is_full_cross

        if l2_is_full_cross(cov):
            # Full crossing is generated by the engine. exclusions are cells
            # already proven impossible. Empty means none are proven yet; Solve
            # still has to classify the cartesian product.
            exclusions = l2_block.get("exclusions")
            if exclusions is None:
                exclusions = []
            if not isinstance(exclusions, list):
                errors.append("coverage.L2 exclusions must be a list")
            else:
                for idx, row in enumerate(exclusions):
                    owner = f"coverage.L2.exclusions[{idx}]"
                    if not isinstance(row, dict):
                        errors.append(f"{owner} is not a mapping")
                        continue
                    parts = row.get("partitions")
                    if not isinstance(parts, dict) or len(parts) < 2:
                        errors.append(
                            f"{owner}: partitions must map >=2 Dimensions to partition ids"
                        )
                        continue
                    for did, pid in parts.items():
                        did_s, pid_s = str(did).strip(), str(pid).strip()
                        if did_s not in dim_ids:
                            errors.append(f"{owner}: unknown dimension {did_s}")
                            continue
                        known = {
                            str(p.get("id") or "").strip()
                            for d in (fence.get("dimensions") or [])
                            if isinstance(d, dict) and str(d.get("id") or "").strip() == did_s
                            for p in (d.get("partitions") or [])
                            if isinstance(p, dict)
                        }
                        if pid_s not in known:
...
def render_plan_prose(fence: dict[str, Any]) -> str:
    ...
    ledger = ledger_counts(fence)
    lv = ledger["levels"]
    cover.append(
        f"- 义务规模：L0 {lv['L0']} + L1 {lv['L1']} + L2 {lv['L2']} + L3 {lv['L3']}"
        f" = {ledger['total']} 条（引擎展开）"
    )
    if ledger["l2_mode"] == "full_cross":
        cover.append(
            f"- L2 全交叉：名义 {ledger['l2_nominal']} − 排除 {ledger['l2_excluded']}"
            f" = 交 Solve 证明 {ledger['l2_obligations']}"
            f"（{ledger['l2_exclusion_rules']} 条 exclusion）"
        )
    oracle = fence.get("oracle") or []
    if oracle:
