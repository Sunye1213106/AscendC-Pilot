...
def init_main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "KB defaults to <算子仓>/.ascendc-pilot/uo. Missing KB → uo_init_required."
        )
    )
...
def plan_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="算子仓（含 .ascendc-pilot/）。也可传 KB 路径。",
    )
    parser.add_argument("--op-name", default="", help="Optional if uniquely inferable from KB / .ascendc-pilot/uo/")
    parser.add_argument(
        "--level",
        default="L0,L1",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional .ascendc-pilot/uo. Used to resolve project_root/op-name.",
    )
    parser.add_argument(
        "--lexicon-seed",
        type=Path,
    ...
    try:
        ...
        payload = {
            ...
            "required": {
                ...
                "preferred": "tg-init confirmed → realization under .ascendc-pilot/tg/",
