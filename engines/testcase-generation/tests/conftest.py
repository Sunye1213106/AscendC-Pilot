...
def write_uo_fixture_yaml(path: Path, data: Any) -> None:
    """Seed UO KB fixtures in tests without TG write isolation.

    Production TG code must still use ``write_yaml`` (refuses ``.ascendc-pilot/uo``).
    """
    path = Path(path)
