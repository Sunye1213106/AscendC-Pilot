...
def assert_tg_write_path(path: Path | str, *, out_root: Path | None = None) -> Path:
    """Refuse writes under `.understand-operator` (UO graph). Optionally require under out_root."""
    resolved = Path(path).expanduser().resolve()
    if ".understand-operator" in resolved.parts:
        raise IsolationError(
            f"TG isolation: refuse write under $UO_ROOT (.understand-operator): {resolved}. "
            "CSV mapping / bind artifacts must stay under $OUT_ROOT (.testcase-generator)."
        )
