...
def init_main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {
                "status": "deprecated",
                "message": "tg-init is deprecated; use tg-plan (intake + extract + plan). Running legacy intake only.",
            },
            ensure_ascii=False,
            indent=2,
        ),
...
def plan_main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {**_plan_summary(result), "manual_gate": ["approve", "reject", "suggest"], "next": "AskQuestion approve|reject|suggest; approve then immediately tg-solve"},
            ensure_ascii=False,
            indent=2,
        )
    )
...
def solve_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Solve approved plan with SMT and emit fag_debug_tools-compatible CSV")
    parser.add_argument("project_root", type=Path)
    parser.add_argument("--op-name", required=True)
    parser.add_argument("--timeout-ms", type=int, default=5000)
    parser.add_argument("--level", default="", help="Level or comma-separated levels to solve from plan/levels/<level>. Omit for top-level plan.")
...
def _plan_summary(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "approval": f"python -X utf8 -m testcase_agent.review_checkpoint <project_root> --op-name <op_name> --level {level} --decision approve",
    }
