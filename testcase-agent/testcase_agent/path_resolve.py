...
def resolve_operator_project_root(path: Path) -> Path:
    """Accept op package, `.understand-operator`, or `.understand-operator/<op>`."""
    ...
    if (root / ".understand-operator").is_dir():
        return root
    return root
...
def infer_op_name(project_root: Path, explicit: str | None = None, kb_hint: Path | None = None) -> str:
    ...
    uo = project_root / ".understand-operator"
    if uo.is_dir():
        children = [p.name for p in uo.iterdir() if p.is_dir() and not p.name.startswith(".")]
        if len(children) == 1:
            return children[0]
        if children:
            raise ValueError(f"Multiple ops under {uo}: {children}. Pass --op-name explicitly.")
    raise ValueError(f"Cannot infer --op-name from {project_root}. Pass --op-name.")


def normalize_contract_root(path: Path) -> Path:
    """Accept realization/, .testcase-generator/<op>/, or a dir that contains realization/."""
    root = path.expanduser().resolve()
    if not root.exists():
    ...
    # If user passed .testcase-generator/<op>
...
def install_contract_into_project(project_root: Path, op_name: str, contract_root: Path) -> Path:
    """Copy/reuse contract artifacts into <project>/.testcase-generator/<op>/realization/."""
    src = normalize_contract_root(contract_root)
    validate_contract_dir(src)
    dest_dir = realization_paths(project_root / ".testcase-generator" / op_name)["dir"]
...
def resolve_plan_paths(
    *,
    ...
    if raw_project is None:
        raise ValueError(
            "OPERATOR_ROOT_REQUIRED: pass project_root (算子仓) or --kb-root (.understand-operator)."
        )
...
# Backward-compatible helpers used by older tests/callers
def realization_map_exists(project_root: Path, op_name: str) -> bool:
    return (project_root / ".testcase-generator" / op_name / "realization" / "realization_map.yaml").is_file()
