...
自然语言任务：Primary 先写 OpenCode Todo（have→want），再按格 `pilot_run`。只有「获取 PR 代码」才 `workflow=auto`（reserved `goal-intake`：隔离 clone + 返回事实）。clone 后主控用只读 git 与 `scan-architectures` 选定算子×架构，再 `pilot_run uo-init`。双轴审查 Task 用原文 ACK 后返回 Primary，勾 Todo 再跑下一格。Host 不得把 `continue_goal` 自动开成下一个用户 slash。有 PR URL 时在 OpenCode 打开目录下新建文件夹 clone；无 URL 才吃本地 diff。已经是算子包（含 `op_host/`）时不再嵌套 clone。**空打开目录不落 `.ascendc-pilot`**，控制面只写在选定的算子工作目录。显式 `workflow=<id>` 一次只跑该工作流。查询不进该环。没有独立 change-impact 工作流：问变更影响 = 带着 diff 做 `/uo-query`。
...
User intent -> pilot_run / acp start
            ...
            -> acp dispatch-result   # native Task 原文 ACK；齐了则 done 回 Primary，勾 Todo 再 pilot_run 下一格
```
...
查询路由在主控。简单查询直接调用 `pilot_cli` `uo-query`，首屏即答案；复杂查询同一轮委派 `Task`。拆路算法在 `pilot-control`。不要 `pilot_run workflow=uo-query`，禁止仅为问题分类而委派子代理。
...
```text
主控 ——简单查询直接 `pilot_cli`；复杂查询同一轮 Task（拆路见 pilot-control）
