...
`/tg-init` 用于在**定稿 KB** 与测试工具之上，生成 `.ascendc-agent/tg/` 的 **confirmed** realization：thin contract、KEY 语义绑定、域确认与 audit。
...
变量：`UO_ROOT=$PROJECT_ROOT/.ascendc-agent/uo`；`OUT_ROOT=$PROJECT_ROOT/.ascendc-agent/tg`。
...
| UO 查询 | `skills/uo-query/SKILL.md` |
...
* 合法 skip：`skills/tg-init/references/legitimate-skips.md`
* UO skill：`skills/uo-query/SKILL.md`
