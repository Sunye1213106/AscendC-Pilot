...
控制面权威在 Harness `workflows/specs.py`。运行时指令由 Composer 从
`skills-src` / `prompts-src` / `agents-src` 组合生成。
...
| uo-code-review | /uo-code-review | 基于 KB 的代码审查 |
| tg-init | /tg-init | 测项合同与绑定 |
| tg-plan | /tg-plan | 覆盖义务与人工批准 |
| tg-solve | /tg-solve | Z3 求解与投影 |
| operator | /operator | `harness route` 别名 |
