...
> **历史决策记录**（非运行时状态机）。Agent / Skill 执行请以 `acp next`、`workflows/specs.py`、`docs/overview/workflows.md` 为准。  
> 本文整理 Pilot 控制面收口计划；只记录当前有效入口和运行边界。
...
- **Pilot 独占**：状态、合法边、门禁、义务（obligations）、收据（receipts）、熵减、完成判定
- **Skill / Prompt**：只做当前动作的领域方法或单次有界任务，不得自管阶段、不得宣布 done
- **引擎复用**：`engines/understand-operator`、`engines/testcase-generation` 核心算法与确定性校验直接挂接，不重写
- **OpenCode**：同级 Tab（`mode: primary` Markdown Agent）+ 独立 `opencode-plugin/` 授权钩子
- **Skill**：最小模板编译（`skills` → `generated/`）
...
| 主题 | 决策 |
|---|---|
| OpenCode 形态 | 同级 Tab：`mode: primary` 的 AscendC-Pilot；与 Plan/Build 并列，Tab 切换 |
| OpenCode 安装 | **只装** `~/.config/opencode/agents/ascendc-pilot.md`；**默认不**合并用户 `opencode.json`；仅当用户显式要求才设 `default_agent` |
| Plugin 位置 | 仓内唯一源：`opencode-plugin/` → 安装到 `~/.config/opencode/plugins/`；`templates/` 不放 Plugin 实现 |
| Skill | **组合式编译**：`skills/{policies,capabilities,actions,roles,workflows}` + `prompts` + `agents` → `generated/<host>/{skills,agents,prompts}`；install 只装 generated |
| `/operator` | 仅为 `acp route` 别名；路由规则只存在于 Pilot，禁止 Python Router / Skill / Prompt 各维护一份 |
| Gate 失败 | **不**立即 `blocked`；进入 `rework_required` / `human_required`，保持当前 phase，由 `acp next` 返回返工动作 |
...
```mermaid
flowchart TD
  user[User NL or slash] --> tab[AscendC-Pilot primary]
  tab --> route[acp route]
  route --> next[acp next]
  ...
  checker -->|"机械性动作"| adv[acp advance]
  referee --> adv
  adv --> complete[acp complete]
  plugin[opencode-plugin Hook] -.->|"authorize acp only"| tab
```
...
| 层 | 负责 | 禁止 |
|---|---|---|
| Pilot | 状态、合法边、权限、门禁、收据、义务、熵减、完成 | 领域算法 |
| Skill（generated） | 当前动作的领域分析方法 | 自管阶段 / 宣布 done / 完整 workflow 自推进 |
| Prompt | 单次有界任务 | 状态机 |
| Producer | 候选产物 | 写裁判 verdict / 改 Pilot state |
...
| 命令 | 行为 |
|---|---|
| `acp start` | 只进 `entry_state`；生产路径无任意 `--phase`（测试可用 force） |
| `acp next` | 当前 `label_zh`、status、open obligations、允许 actions、last_failure |
| `acp advance` | 仅合法边 + 所需 checker（及可选 referee）通过 |
| `acp rework --reason <code>` | 沿显式 rework 边 |
| `acp complete` | 仅 terminal-ready + complete_gates；唯一合法 `passed` |
| `acp route` | NL/slash → `workflow_id`（`/operator` 与此同义） |
...
Pilot Adapter 把领域产物归一为 obligation；Workflow Spec 不写死所有 ID。动态义务在每次 `next`/checker 后刷新。
...
只能由 Pilot 包裹执行后写入，禁止 Agent 自写「我执行过」的 YAML。字段至少含：
...
| 动作 | 对象 |
|---|---|
| **保留** | `engines/understand-operator`、`engines/testcase-generation` 核心算法与现有确定性校验函数 |
| **升格为权威** | `pilot/ascendc_pilot/workflows/` → 完整 Workflow Spec |
| **瘦身** | `skills/*/SKILL.md`、`prompts/**/workflow.md`、`docs/*-workflow.md` — 删除独立状态机，改为读 acp status/next |
...
- **KEY receipt**：必须 Pilot 签发的完整 receipt；禁止「有 patch 即可」
- **confidence**：patch reason → 确定性脚本生成报告 → referee 只审
- 删除 `contracts/` 初始化与 `testcase_contract`
- ownership：`uo-orchestrator` → `pilot-uo-adapter` + `deterministic-uo-engine`
- 删除 tg-csv-contract 可选写 UO；改 TG proposal，由独立 UO workflow 合并

### 运动员 / 裁判隔离

- Producer 可写面 ∩ Referee 可写面 = ∅
- 父代理 / AscendC-Pilot **不得**直接写正式 IR/summary/review（经 acp authorize）
- Referee 产物必须含 `agent` + `run_id`；Pilot gate 校验
...
```text
安装目标：~/.config/opencode/agents/ascendc-pilot.md
frontmatter：mode: primary、description、permission
```

仅当用户显式要求时才设置 `"default_agent": "ascendc-pilot"`。

### Plugin

```text
仓内：  opencode-plugin/（如 ascendc-pilot.ts）
安装后：~/.config/opencode/plugins/
```

`tool.execute.before` 调 `acp authorize`（校验 workflow / phase / action / agent / write path）。
...
禁止 primary 直接 `python build_layered_kb.py` / `tg-solve` 等。尚未被 Pilot 包装的领域 CLI 最多 `ask`，不可 `allow`。领域执行由 Pilot 再调 UO/TG Engine。

### 威胁模型（必须诚实表述）

在 AscendC-Pilot primary 模式内，permission + Plugin 阻止常规越级。

**不能**宣称 OS 级绝对禁止绕过：用户可 Tab 回 Build、终端直改、UI 直接 @ subagent。OpenCode 对部分 subagent/batch 钩子仍有平台限制。

正式完成仍依赖：**Receipt + Checker + `acp complete`**；绕过路径无法获得 `passed`。
...
```text
/operator → acp route → workflow_id
AscendC-Pilot → 同一 acp route
```
...
| 2. UO/TG 契约接入 | TG engine gate adapters、UO 门禁收紧、confidence 三层、写权隔离、四类 Spec Hash、contracts 删除 | 七个 workflow 均可经 Pilot CLI (acp) 走完（固件/桩） | completed |
| 3. Skill 单一来源 | skills、compiler、generated、/operator 别名、文档去双控制面 | install 只部署 generated；Skill 无独立状态机 | in_progress |
| 4. OpenCode 模式 | `ascendc-pilot.md` primary、acp-only bash、plugin authorize、可选 compaction | Tab 可见；非法直调被拦；不改用户 opencode.json | pending |
...
- Receipt 仅 Pilot 签发；含 actor_type 与哈希
- OpenCode：Markdown primary Agent + plugins/；安装不合并 opencode.json
- Primary bash 仅 `acp *`；正式完成只认 Receipt + Checker + complete
...
| TG 阶段升格后 Skill 叙事分叉 | Pilot Spec 为唯一源；generated Skill 只引用 `acp next` |
...
- Pilot 使用摘要：[`pilot/README.md`](../pilot/README.md)
- Skill/Prompt 原则（需随本迭代同步「Pilot 管状态、Skill 管领域」）：[`skill-and-prompt-principles.md`](./skill-and-prompt-principles.md)
- 各领域流程文档：`docs/uo-*-workflow.md`、`docs/tg-*-workflow.md`（迭代中改为「读 acp status/next」，删除双控制面表述）
