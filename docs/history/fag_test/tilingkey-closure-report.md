...
| 文件 | 内容 | 状态 |
| --- | --- | --- |
| `evals/fixtures/fag/fag_arch35_reachable_cases.csv` | 4226 行 witness（4169 声明可达 + 57 未声明），每行附完整输入与 19 维 | **在库** |
...
该表已改写成与 `TEST/fag_debug_tools/data/FASG.xls` 同构的用例表（原转换脚本已删除）。列顺序与 xls 一致：
