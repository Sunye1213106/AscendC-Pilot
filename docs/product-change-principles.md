...
> 联调案例：`docs/uo-init-联调问题与修复.md`。组合式分层：`docs/skill-and-prompt-principles.md`。
...
5. **改完 compose + 单测 +（需要时）联调文档**；安装侧 `refresh-opencode.ps1` 后重启才算生效。

## 反模式（禁止）

- 只在 `extract-plan.md` 里写「高置信必须源码比对」，公共 Policy 不改。  
