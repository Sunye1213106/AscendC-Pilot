...
* 大模型只处理新增不确定项（任务 A/B/C；KEY 走 `uo-key-resolve` triage→分流）；
...
* 子代理：`uo-semantic-resolve` · `uo-key-resolve`
...
| 新 gaps | KEY | `tpl_key_triage` → `tpl_key_resolve`（`uo-key-resolve`） | classify / confidence |
