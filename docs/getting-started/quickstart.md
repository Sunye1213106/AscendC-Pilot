...
自然语言要生成用例且带 PR URL：先 Todo 再按格执行。获取代码走 Engine clone；`auto` 回执已唯一钉死 `(算子, architecture)` 时直接用于后续格，不要单独一条「确定算子/架构」Todo。全部 init 在前，再按产物缺口消费：最终产物是用例、审查不是交付物 → `/tg-init` 后直接 `/tg-plan` / `/tg-solve`（`plan_scope` 像 `uo-query` 说清要测什么，Primary 读回答，不写文件，不要中间再派自由 `/uo-query`）；审查才是交付物 → `/ce-review`（若同时还要用例，审查结论留给 `/tg-plan` 的 Planning Context，不再加一轮自由查询）。勾 Todo 后再 `pilot_run` 下一格，不要再调 `auto` 做 intake。`/tg-init` 缺测试仓会问人。需主控派 Task 的格串行。不要按个别措辞选 slash。
