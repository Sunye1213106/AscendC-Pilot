---
name: tg-init
description: 'TG stage-1: thin contract + semantic binding + domain confirm. Parent
  auto-loops uo-query Tasks until csv-closure + audit pass + --confirm. User only
  runs /tg-init (embeds former tg-contract / tg-domain-review).'
argument-hint: <算子仓> --op-name <op> --test-script-root <测试工具> | --merge-uo-resolve
  | --verify-csv-closure | --confirm
...
测试工具 + **定稿 KB** → `.ascendc-agent/tg/` **confirmed** realization
...
- 语义绑定：Task Follow `skills/uo-query/SKILL.md`（并行 cap=8）
- 派发合同：`prompts/tg/init/dispatch.md`；命令块：`prompts/tg/init/workflow.md`（若无则 `prompts/init/workflow.md` 仅 UO）
