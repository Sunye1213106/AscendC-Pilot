---
name: uo-query
description: 基于定稿算子 KB 做语义问答或 TG 绑定。/uo-query 或 Task 内 Follow 本 skill。 主路径 uo_kb_query.py
  查 kb_graph.sqlite；未达 high 再 MCP 举证。
...
- 适用：`/uo-query`、KB Q&A、TG bind、建库**完成后**的复杂 KEY 升级（可经 `uo-key-resolve` 调用本 CLI）
- 不适用：`/uo-init` **建库期** KEY 闭合（用 `uo-key-resolve` triage→分流）；无 KB / stale KB（先 init/update）
...
- `/uo-init` 建库期派本 skill 修 `input_derivable`（用 `uo-key-resolve` triage→分流）
- 默认每个 KEY 一个 subagent（须先 triage；simple 可打包）
...
- **Exit：** 有可引用的实体/邻接/约束
- **Failure：** CLI 路径错误 → 用 `$PLUGIN_ROOT/engines/uo/uo/scripts` 重试一次；仍失败 → `TOOL_FAILURE`（禁静默 Grep）
...
- 定稿后 TG 可读 `input_derivable_gaps` 作证据，闭合写 OUT_ROOT；**建库期** gaps 用 `uo-key-resolve`，不得改派本 skill
