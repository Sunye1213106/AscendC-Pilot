...
| | |
|---|---|
| Entry | Phase0 完成（`scope_confirmed` + `indexed_via: mcp`） |
| Exit | `build_layered_kb` 成功；存在 `ir/input_derivable*.yaml` / `ir/unresolved.yaml` |

机制：**CBM 符号检索 + 源码正则 / 花括号定界函数体**（非完整 C++ AST）。  
人读 Step 明细：`docs/uo-init-workflow.md` Phase 1。
