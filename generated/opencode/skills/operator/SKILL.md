---
name: operator
description: 可选助手：列出可用 Pilot workflow skill，或把 /uo-init 等 slash 转给 acp route。 自然语言意图请直接加载对应
  workflow skill（如 uo-init），不要依赖本 Skill 做口语路由。
---
...
3. **禁止**维护第二套关键词/路由表；口语建库等意图直接加载 `uo-init` 等 Skill。
