---
name: uo-init
description: 端到端构建 AscendC 算子分层 KB。/uo-init 或要求建库时用。 Phase0 人工确认 → 入口/plan（脚本+有界 LLM）→
  分层抽取 → resolve → 门禁 → 导出。
...
空仓库 / 未建库 → **定稿** `$PROJECT_ROOT/.ascendc-agent/uo/`（integrity + kb-review 通过）。
...
- 建库期子代理：**`uo-semantic-resolve`**（A/B/C）、**`uo-key-resolve`**（KEY triage/resolve）、**`uo-confidence-review`**（原因裁判）、**`uo-kb-review`**
- Phase0 **禁** `explore`/`generalPurpose` 预扫；范围只信 `macro_scope_scan`
- 默认 **tests/examples 不进**候选；AskQuestion 必须转述脚本 cpp/h 计数表
- 闭合 KEY 必须 `confidence: high`；复杂 KEY 走 triage→分流（`uo-key-resolve`）；非 high 原因须经 **confidence-review**；CBM 仅 MAY
- 抽取机制：CBM + **花括号定界函数体 + 定向正则**（非完整 C++ AST）；KEY 路径 CBM 仅 MAY
- 幂等：同 scope 重跑可覆盖未人工锁定产物；禁静默改用户已确认 scope
- 语言：`prompts/common/language.md`（产物 reason/findings 中文）
- 完成态：仅 `harness complete`；禁止 Agent 自行宣布 done
...
- Extract：脚本入口 →（条件满足）LLM 任务 A → 脚本扩 plan → **必须** LLM 任务 C → `build_layered_kb`
- Gaps open / confidence≠high → **KEY triage → uo-key-resolve** → `classify_input_derivable` → `check_final_confidence` →（need_llm）**`uo-confidence-review`** → **`harness validate-key-gates`** → **`harness complete`**
- 源码证据：KEY 主路径读 Host `file_path`；CBM 见 `prompts/common/cbm.md`（MAY）
...
- 自动 `continue` Phase0；Phase0 派 explore 预扫；整盘搜脚本
- 写 `contracts/**`；dump `operator_graph` / 完整 `exhaustive`；本地 CBM CLI
- 低置信伪标 `true`/`high`；跳过 `--write` / `--confirm-patch` 空跑 LLM
- 跳过 key-triage / 手写 accepted 顶替 key-resolve
- 默认「每个 KEY 一个 subagent」（须先 triage；simple 可打包）

## Workflow

**控制面：** 只通过 `harness start/next/advance/rework/complete` 推进；下方领域步骤是 action 方法，不是独立状态机。

变量：`SCRIPT_DIR=$PLUGIN_ROOT/engines/uo/uo/scripts`；`UO_ROOT=$PROJECT_ROOT/.ascendc-agent/uo`。
先：`harness start uo-init --project $PROJECT_ROOT`。  
阶段推进：`harness advance <phase>`；收工：`harness complete`（唯一 pass）。  
...
- **Actions：** 见 `references/resolve.md`（任务 B → KEY triage/resolve → classify → confidence → export → integrity）
...
| 适合脚本 | 适合 LLM |
|---|---|
| 范围扫描、入口候选、plan 扩面、分层抽取、classify、export、integrity | 入口消歧（A）、plan 角色（C）、FP 抽样（B · `uo-semantic-resolve`）；KEY triage/resolve（`uo-key-resolve`） |

- KEY resolve 并行 Tasks cap≈8；约 ≤12 tools/批
- KEY 主路径：Host `file_path`；CBM 仅 MAY；证据须 `path:line`
- 建库期不派 `/uo-query` 做 KEY 闭合
