...
description: 'UO semantic producer for action-scoped extraction planning and semantic
  adjudication. Evidence path: CBM MCP (search_graph → get_code_snippet) or `acp cbm
  lookup`, then windowed Read on confirmed-scope sources — never whole-file dumps.
  Writes limited by Action lease.
...
UO semantic producer for action-scoped extraction planning and semantic adjudication. Evidence path: CBM MCP (search_graph → get_code_snippet) or `acp cbm lookup`, then windowed Read on confirmed-scope sources — never whole-file dumps. Writes limited by Action lease.
