...
2. 用插件 `pilot_cli`（command=`uo-query --project <op> [--architecture arch35]`，不要前导 acp）加上一种参数形态：标识符、`Dim=V`、`--file --line`、或无参数索引（或本能力脚本 `uo_kb_query.py`，它只转发到该 CLI）。不要传 `--mode`。
