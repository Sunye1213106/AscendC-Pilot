...
SCRIPTS = ENGINE / "uo" / "scripts"

helper = r'''"""Deterministic C++ receiver-type facts and one-hop return propagation.
...
def build_receiver_type_facts(
    ...
    *,
    ...
    for fn in functions:
        ...
        for statement, offset in _iter_semicolon_statements(fn.body_text or ""):
...
(SCRIPTS / "receiver_type_facts.py").write_text(helper, encoding="utf-8")

graph_path = SCRIPTS / "function_call_graph.py"
graph = graph_path.read_text(encoding="utf-8")
import_marker = "from uo.scripts.macro_regions import analyze_macros\n"
import_text = '''from uo.scripts.receiver_type_facts import (\n    ReceiverTypeFacts,\n    build_receiver_type_facts,\n    infer_receiver_type as infer_receiver_type_from_facts,\n)\n'''
if import_text not in graph:
    if import_marker not in graph:
        raise SystemExit("receiver import marker missing")
    graph = graph.replace(import_marker, import_marker + import_text, 1)

field_marker = "    internal_classes: set[str] = field(default_factory=set)\n"
field_text = field_marker + "    receiver_type_facts: ReceiverTypeFacts | None = None\n"
if "receiver_type_facts: ReceiverTypeFacts" not in graph:
    if field_marker not in graph:
        raise SystemExit("facts field marker missing")
    graph = graph.replace(field_marker, field_text, 1)

return_marker = "    return facts\n\n\ndef build_call_edges_for_functions(\n"
return_text = '''    facts.receiver_type_facts = build_receiver_type_facts(\n        functions,\n        facts.source_text_by_file,\n        official_contracts=facts.official_contracts,\n    )\n    return facts\n\n\ndef build_call_edges_for_functions(\n'''
if "facts.receiver_type_facts = build_receiver_type_facts(" not in graph:
    if return_marker not in graph:
        raise SystemExit("collect facts return marker missing")
    graph = graph.replace(return_marker, return_text, 1)

start = graph.find("def _infer_receiver_type(\n")
end = graph.find("\n\ndef _receiver_object(", start)
if start < 0 or end < 0:
    raise SystemExit("receiver function markers missing")
replacement = '''def _infer_receiver_type(\n    site: CallSite, caller: FunctionDefinition, facts: CallResolutionFacts\n) -> str:\n    receiver_facts = facts.receiver_type_facts\n    if receiver_facts is None:\n        receiver_facts = build_receiver_type_facts(\n            [caller],\n            facts.source_text_by_file,\n            official_contracts=facts.official_contracts,\n        )\n    return infer_receiver_type_from_facts(\n        site,\n        caller,\n        receiver_facts,\n        official_contracts=facts.official_contracts,\n    )\n'''
graph = graph[:start] + replacement + graph[end:]
...
'''
(ENGINE / "tests" / "test_receiver_type_propagation.py").write_text(test, encoding="utf-8")
print("applied receiver type facts and one-hop return propagation")
