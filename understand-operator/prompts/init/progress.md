...
| 2 | 扫描并提案分析范围（含向上发现 common） | #1 | `scope_proposal.yaml` 可读 |
| 3 | 等待确认分析范围（硬门禁） | #2 | 用户 `continue` 且 checkpoint 写入；**禁自动 continue** |
...
| 6 | Resolve：按 `references/resolve.md` Step 1–6（残留+断边+门禁+导出+integrity） | #5 | unresolved 清空；confidence_gate pass/reported；integrity pass |
| 7 | KB 产物审查（uo-kb-review） | #6 | `kb_product_review.yaml` verdict=pass → human_views |

## 硬规则

- 内部预检 / 路径解析 **不**单独占 Todo
- #3 未确认 **不得**进入 #4（索引）
- #6 含任务 E 与 `check_final_confidence`；不得跳过 gaps open
