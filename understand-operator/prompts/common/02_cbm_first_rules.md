...
The operator KB is a **variable / entity map** for fast routing. When you need
source proof after looking up an id in the KB, prefer MCP
`codebase-memory-mcp` (`search_graph` / `get_code_snippet` / `search_code` /
`trace_path`) over dumping whole files into the chat context.
...
- Moving the KB to a parent workspace that contains many operators.
...
KB location is always `$PROJECT_ROOT/.understand-operator/$OP_NAME` where
`$PROJECT_ROOT` is the **operator package subdirectory**.
