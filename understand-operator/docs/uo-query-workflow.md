...
* 建库期不得用本流程代替 `uo-semantic-resolve` 任务 E。
...
| --- | --- |
| 定稿 KB 上的问答 / TG bind | `/uo-init` 建库期断边（用任务 E） |
...
`UO_ROOT=$PROJECT_ROOT/.understand-operator/$OP_NAME`；TG 交付根为 `$OUT_ROOT=$PROJECT_ROOT/.testcase-generator/$OP_NAME`。
...
* TG 细则：`../testcase-agent/skills/tg-init/references/tg-uo-query-escalation.md`（TG Task）
* UO 回流：`uo/scripts/apply_resolution.py`（仅 UO staging）
...
1. 父代理按 KEY 派并行 subagent（cap≈8），每个 Follow 本 skill；
2. 子代理仍走 Step 2→6（先 sqlite）；
3. 父代理合并：
   * TG → `--merge-uo-resolve`（只读 `uo_query_resolve/`）；
   * UO staging → `apply_resolution`。

不得与建库期 `input_derivable_gaps` 混用。
