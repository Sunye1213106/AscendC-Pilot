...
不得派发 `/uo-query`，避免建库过程反向依赖尚未完成的 KB。
...
输入为项目目录、算子名和目标架构；输出为 `runs/<id>/phase0/scope_proposal.yaml`，包含候选文件及纳入理由。
...
禁止自动选择 `continue`。用户选择 `stop` 后立即终止；选择修改或补充时重新生成确认范围。

**输入 / 输出**

输入为 `scope_proposal.yaml` 和用户决策；输出为 `scope_confirmed.yaml` 及门禁记录。
...
Phase 2 负责解析 unresolved、补全图中断边、检查置信度，并导出正式知识图谱。

## Step 1：解析残留 unresolved

**关键文件**

* 提示词：`prompts/init/tpl_residual.md`
* 子代理：`uo-semantic-resolve`
...
大模型处理简单变量别名、短赋值链、局部布尔表达式和明确字段来源。每次只处理有限数量的简单 false positive；复杂 TilingKey 或多层条件进入 `escalate_keys`。

禁止无证据闭合、使用 `/uo-query` 或将猜测标为 high。
...
* 提示词：`prompts/init/tpl_input_derivable.md`
* 子代理：`uo-semantic-resolve`
* 图规则：`skills/uo-init/references/resolve.md`
...
* 字段写入与读取对应；
* 简单派生变量关系；
* Host 与 Kernel 之间的桥接边。

只有充分证据支持的关系才能标记为 high。不得将 Kernel 循环状态、运行时索引或编译宏标为外部输入可推导。

单次处理数量上限为 8。

**输入 / 输出**

输入为 layered IR、bridge、gaps 和源码证据；输出为 gap patch、闭合边、开放边及置信度。
