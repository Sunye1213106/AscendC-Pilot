...
1. **禁止**打开 `ir/operator_graph.yaml`、历史 `contracts/**` 或
   `cross_layer/impact_graph.yaml`，除非所有热路径都未命中事实。
...
7. **`/uo-init` 建库期禁止派发 uo-query**；建库 `input_derivable` 断边用 sen 任务 E。定稿后 TG bind 可读 gaps 作证据，闭合只写 `$OUT_ROOT`。
