...
def resolve_repo_source_path(repo_root: Path, file_path: str) -> Path | None:
    ...
    - ``.understand-operator/.../cbm/index_stage/.../op_host/...``
    ...
    for marker in _SOURCE_MARKERS:
        ...
        # Staging mirror under .understand-operator/<any>/cbm/index_stage/<any>/<rest>
        uo = repo_root / ".understand-operator"
        if uo.is_dir():
            for op_dir in uo.iterdir():
