...
**每一步「查代码 / 找符号 / 看实现 / 跟调用链」的第一个动作都必须是 `cbm_query.py`。** 适用于宿主与所有 subagent。

1. 先 `cbm_query.py`（简写参数优先）
2. 从 stdout 提取符号、文件、行号
3. **只有**满足以下之一才允许 `Read` 源码，且必须**带行号小范围**：
   - CBM 已返回具体 file+行号，需核对该片段；
   - 宏展开 / 模板实例化 / 字符串属性 CBM 拿不全；
   - CBM 查询返回空或报错（须先在 journal/evidence 记录该次查询）。
4. **禁止**未查 CBM 就整文件 `Read`；**禁止**以「快 / 稳 / 顺手」为由先整文件读源码。想整文件读时，先改成一次 CBM 查询。
