...
`confirmed` means the claim is directly supported by exact source-location
anchors. Formal `sources` store only `id`, `file`, `symbol`, `span`, and
`anchor_kind`; they never store source text, code hashes, file hashes, encoding,
newline, or BOM metadata. `not_applicable` means the category was checked and
does not apply. `unresolved` means the claim cannot be proven.
