...
Do not read whole `.cpp` / `.h` files. Read source only with line scope after CBM gives a file+line, when macro/template/string context is incomplete, or when a CBM query returns empty/error and that query is recorded.
