...
from understand_operator._operator.repair_controller import MAX_ATTEMPTS, mark_repair_completed, read_repair_state, record_repair_attempt
...
def run_candidate_batch(repo_root: Path, op_name: str, batch_path: Path) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        state = read_repair_state(uo_root, run_id, task_id)
        ...
        if errors:
            ...
            repair = record_repair_attempt(uo_root, run_id, task_id, owner, target, str(batch_path), error_dicts)
            status = "exhausted" if repair else "retrying"
            return 2, {"status": status, "phase": phase, "errors": error_dicts, "repair": repair}
        state = mark_repair_completed(uo_root, run_id, task_id, owner, target, str(batch_path))
