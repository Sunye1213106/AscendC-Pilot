...
1. 展示提案摘要：**必须原样转述** `macro_scope_scan` stdout / `summary.included_layers` 的 INCLUDE 与 EXCLUDE 表（每层 `.cpp` + `.h`）
2. 说明默认排除：`tests` / `examples` / `ut` / `st`；其它 `arch*` 目录已按 `--architecture` 过滤
3. AskQuestion 等待用户
4. 写入 checkpoint：
...
| `manual_supplement` | 人工补文件列表后再次确认（可用 `--seed` 加回 tests 等） |
| `stop` | 结束 init；保留已写目录，不索引 |

## Acceptance

- checkpoint 反映真实用户选择
- AskQuestion 消息含脚本计数表（非 LLM 自编 op_host 数）
- 无确认却进入索引 → 流程违规，必须停
