...
变量：`SCRIPT_DIR=$PLUGIN_ROOT/engines/uo/uo/scripts`；
`UO_ROOT=$PROJECT_ROOT/.ascendc-agent/uo`。禁全盘搜脚本。
...
# MUST：原样转述 stdout INCLUDE/EXCLUDE 计数表（cpp+h）→ AskQuestion（scope_menu）→ 禁自动 continue
# MUST NOT：Phase0 派 explore/generalPurpose 预扫
...
收窄初次提案可用 scan 的 `--replace-initial`。默认排除 tests/examples。  
...
2. `escalate_keys` / gaps `open` / confidence≠high →
   - 先 `tpl_key_triage`（一次）→ `ir/key_triage.yaml`
   - 再 `tpl_key_resolve`：complex 单 KEY / simple 打包（≤6）；CBM=MAY；禁默认每 KEY 一 agent
3. 跑门禁与导出：
...
- 写入遵循 `ownership.yaml`；思考/话术中文（`runtime.md`）；派发模板正文中文
- 禁 dump `operator_graph` / 完整 `testcase`；KEY 主路径读 Host；CBM 仅 MAY
- 子代理：semantic-resolve + **key-resolve** + kb-review（`dispatch.md`）；Phase0 禁 explore
