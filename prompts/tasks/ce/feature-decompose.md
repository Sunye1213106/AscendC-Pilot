...
3. 不确定内容标记 `UNRESOLVED`；仅写 staged parts / staging，不写 canonical CE 计划。
4. 验收条件要能在后续 `/ce-verify` 用 UT/ST/精度对比/profiling/复测收据关闭，不要写「看起来没问题」。
</instructions>

<output>
写入 `runs/<RUN_ID>/actions/feature_decompose/parts/part_0.yaml` 与同目录 `staging.yaml`，并保持草稿合同字段完整。
</output>
