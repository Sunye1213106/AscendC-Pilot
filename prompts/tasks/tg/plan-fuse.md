<task>
基于本次 `tg/init.yaml` 与已给定的 Planning Context，生成 `plan.md` 草稿。
</task>

<input>
- Init: `<PROJECT_ROOT>/.ascendc-pilot/<ARCHITECTURE>/tg/init.yaml`
- Planning Context: `<PROJECT_ROOT>/.ascendc-pilot/<ARCHITECTURE>/runs/<RUN_ID>/actions/plan_scope/parts/purpose.md`（外加用户意图 / CE plan「测试内容」 / session handoff）
