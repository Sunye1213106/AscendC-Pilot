...
3. 禁止做 schema / YAML 审查。非法组合由引擎 validator 拒绝。
</delta_constraints>

<method>
通读 `parts/harness.yaml` 与 `parts/bind.yaml`。不要写文件。不要打开 harness / columns 的生产 playbook。放行后引擎才 `bind_promote`。两份没到齐不能判。

`inspect yaml` 过了只说明能合并。`PASS` 时引擎会再跑同一 validator，失败是 `BIND_PART_INVALID`。本窗只抽查 validator 不能证明的 provenance。
