...
if str(REPO / "engines" / "tg") not in sys.path:
    sys.path.insert(0, str(REPO / "engines" / "tg"))
...
def test_composer_detects_missing_agent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    from ascendc_harness.workflows import specs as specs_mod
    ...
    agent = REPO / "agents-src" / "tg-semantic-bind.yaml"
...
def test_nondeterministic_actions_have_agents_and_prompts():
    ...
    prompts = REPO / "prompts-src" / "tasks"
...
def test_action_id_propagates_via_active_action(tmp_path: Path):
    ...
    from ascendc_harness.actions.runtime import prepare_action, _write_active_action
    from ascendc_harness.paths import agent_root, ensure_agent_layout
...
def test_semantic_bind_closed_loop_and_stale_rejection(tmp_path: Path):
    from ascendc_harness.actions.runtime import finalize_action, prepare_action
    from ascendc_harness.paths import ensure_agent_layout, tg_root
    from ascendc_harness.state import start_workflow
    ...
    _write(
        tmp_path / ".ascendc-agent" / "context" / "harness_params.yaml",
...
def test_doctor_flags_missing_prerequisites(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    from ascendc_harness.cli import _doctor
...
def test_unresolved_not_auto_completed(tmp_path: Path):
    from ascendc_harness.gates.tg_adapters import gate_bind_progress
    from ascendc_harness.paths import ensure_agent_layout, tg_root
...
def test_plugin_reads_active_action_helpers():
    ...
    text = (REPO / "opencode-plugin" / "ascendc-harness.ts").read_text(encoding="utf-8")
