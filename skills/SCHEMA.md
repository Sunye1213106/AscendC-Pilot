...
6. **模型可见中文。** 判断与步骤用中文。保留英文的只有：路径、YAML/字段名、CLI、裁决枚举（`HIT` / `TARGET_HIT` / `PROVED`）、覆盖档（`L0`–`L3`）、`evidence.kind`。标题与「何时加载」一律中文。禁止 `When to load` / `Gotchas` 当一级标题。
...
切片 HOW 若只属于某一轴，放 `references/<axis>.md`，由 Spec `fanout_axes[].method_ref` 装进该窗的 `method.md`；`refs` 是该窗才拷的一层指针（轴文件禁止 hop）。父窗口 `SKILL.md` 写清各窗交什么、禁止混轴；prepare 不把轴 HOW 拷进父 session。Primary 后序裁判走独立 reviewer action 的 prompt，不装 producer Skill。禁止把多窗正文拼进一份始终装载的 `SKILL.md`，也禁止同一个 Skill 既生产又裁判。

### 禁止

- 复述 Policy / invariant 全文；只点名（如「全局限制见 code-access」）。不把另一份 Skill 当工具语法手册。
