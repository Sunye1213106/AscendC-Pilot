...
1. 若 `intent.yaml` 的 `open_questions` 非空，拒绝进入分解后的计划，要求回到问需求。
2. 验证每个特性都有目标、约束、候选锚点（符号/实体名即可）和可验证的验收条件。
3. 拒绝无候选锚点、越界范围或不可验证的完成条件。名称近似命中只能作为 Tier C 线索。
4. 通过时将审查结论写入 `ce/intent/plan_review.yaml`（含已接受特性清单）。canonical 特性清单由后续确定性动作根据本审查结果写出。
