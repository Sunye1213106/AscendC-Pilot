...
4. **shape 列是 range，且做了双源比较。** `domains.profile` 引用 `tables[].profile`；`domains.operator` 分开写声明面与产品覆盖面。`compare` 枚举与 `call.kind` 合法值由 promote 后的 `validate_init` 闸门；本步判「有没有做比较、叙事是否同一世界」。把一次抽样当成枚举全集 → `REWORK bind`。
...
| 现象 | 下一发 |
| --- | --- |
| 列名对不上入口 / mapping 空 / 发明列 / shape 不是 range | `REWORK bind` |
