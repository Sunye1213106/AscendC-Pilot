...
name: source-proof
description: >
  基于 CodeMap 与源码证明或反驳程序语义命题。证明某条件必然导致某结果、
  某状态不可出现、某字段只能取特定值，或为静态结论建立可审计源码证据时使用。
---
...
6. **结论与证书**：`references/proof-certificate.md`；裁判：`references/referee-replay.md`；新鲜度：`_shared/artifact-freshness.md`。
...
| C++ / 证据 / 完整性 | `_shared/cpp-semantics.md` / `_shared/evidence-quality.md` / `_shared/completeness.md` |
