...
| 复杂 KEY / shape unresolved（`escalate_keys` / bind 缺 expr） | **不要** bare unsolved；走 `complex-unresolved-escalation.md`：先 triage，再 complex 单 KEY / simple 打包 + `branches_for_key`/`affected_shapes`；CBM=MAY |
...
**允许且不需要 MCP：** 只读 `.ascendc-agent/uo/` 下 YAML/MD，以及本 skill / prompts。
