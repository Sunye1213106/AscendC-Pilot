---
name: bind-columns
description: 按调用接口把测试表列绑到算子变量。tg-init 的 columns 切片使用。
---

# 绑定列

只写 `parts/bind.yaml`。本路回答「脚本怎么调算子、每个入参来自哪一列、剩下的列是什么」。不要写 golden / compare / modes，不要读另一路的 `harness.yaml`。列值域以 `tables[].profile` 为准，不要通读 CSV。
...
读：`repo_scan.yaml`（表头、`tables[].profile`）。有仓则**自己打开入口脚本**，不要等 harness。无仓则列来自 Host API，不要发明 CSV 列名。

有仓且 API 入参列 mapping 空 → 本切片失败。`script_meta` 允许无 `uo_id`。`api_arg` / `attr` 缺标识符仍失败。

完成：`call` 已记；API 入参已绑回列；剩余列有 role；domains 做了 profile vs operator 比较。
...
四种形态：无参数索引；一个标识符；`Dim=<维名>` 或 `Name=Value`；`--file --line` 只从上一张卡复制。未从卡片复制 file:line 时禁止 around。有位点时 around 必须可用，不能一刀切禁用。

第一张卡的 `file` / `line` / `extras.writers` 是位点来源。索引 hint 会告诉覆盖列表怎么写、组合过滤怎么写；不要把 `Dim=` 当成 `Name=Value` 的前缀胡写。
...
两路都写完即停。不要 PASS、不要开始 `/tg-plan`。

## 指针

脚本仓与读点：`references/test-script-repo.md`。列绑定边角：`references/column-binding-edge-cases.md`。
