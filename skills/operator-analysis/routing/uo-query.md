...
**禁止** `pilot_run workflow=uo-query`。禁止仅为问题分类而委派子代理。不要为确认协议调用 `--help` 或 Glob 查找 routing。失败用 `pilot_cli` `inspect-failure` / `uo-query --status-only`，不要对 `uo-query` / `start` / `run-action` 再调 `--help`。
...
`pilot_cli` `uo-query` 返回 `UO_PRODUCT_REQUIRED` + `ask_question` 时立刻 AskQuestion（选项原样）：先 `/uo-init`，或回退源码作答。禁止 Glob/dir/tree 找产物。
