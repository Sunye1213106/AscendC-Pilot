...
尺寸列绑本列对应的维/字段，不要绑调用里用它算出来的 kwargs。变长序列列对 seqlens kwargs，不对开关维。

`out_dtype` 若只是 `.to(pttype)` 间接进调用 → `tensor_dtype`，不是 `direct`。若调用根本没有该实参、只进 golden JSON → `unwired`。

## 查图只为闭合链路

仅 `control.status: active` 之后才 `uo-query`。必须先做一次无参查询。不要 `Dim=` 扫目录。缺哪个标识符查哪个。

`uo.id` 填短名 / `canonical`，禁止 `TDF::` 和 `tiling_data_names` 结构名。只碰到相似符号 → `uo.candidate` + `unresolved`，不要升格。kwargs 的 source 列对不上字段时，`uo.id` 填该 `call_args.name`，不要为此 `partial`。

projection（`Input_Layout → IsTnd`）写在 `domains.projection`，不要当 equality 控制、不要当另一列的 `uo.id`。
