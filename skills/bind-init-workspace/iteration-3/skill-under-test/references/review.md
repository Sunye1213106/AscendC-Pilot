...
2. **有仓则 API 入参 mapping 非空。** `api_arg` 必须有脚本读点与短名 `uo_id`（`''` 等于没绑）。改写列（本身不在实参里、只改另一列取值）应是 `feature`。尺寸列若只是用来算出另一个实参，应绑本列身份，不绑被算出的 kwargs 名。`feature` 头文件有开关字段则应有 `uo_id`。`script_meta` 不得假造标识符。不要把传进调用的列标成 `attr`。有仓却 API 列空 → `REWORK bind`。`inspect yaml` 过了只说明能合并。
3. **没有发明列。** bind 列名必须来自 scan 表头或（无仓时）Host API。不要把计划意图里的场景 id 当成列。
4. **shape 列是 range，且做了双源比较。** `domains.profile` 引用 `tables[].profile`；`operator` 填该列短名。`operator` 空却 `compare=match`，或把模板开关 `Dim=` 写进「只在该开关下才出现」的列 → `REWORK bind`。把一次抽样当成枚举全集 → `REWORK bind`。
