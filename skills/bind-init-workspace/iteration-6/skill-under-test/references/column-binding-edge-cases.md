...
尺寸列绑本列对应的维/字段，不要绑调用里用它算出来的派生 kwargs（scale 一类），也不要绑张量操作数名或 `*TemplateNum`。若该列出现在标量 kwargs（`head_num` 等）的 `sources[]`，`uo.id` **等于** 该 `call_args.name`，不要改走维字段候选、不要只放 `candidate`。

`out_dtype` 若只是 `.to(pttype)` 间接进调用 → `tensor_dtype`，不是 `direct`。若调用根本没有该实参、只进 golden JSON → `unwired`（不要因为图上有 Out/Input DType 开关就标 active）。
