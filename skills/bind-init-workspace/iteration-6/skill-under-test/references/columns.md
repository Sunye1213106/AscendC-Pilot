...
- **标量 kwargs 的 source 列 `uo.id` = 该 `call_args.name`。** 判断看 arg **名字**（`head_num` / `keep_prob` / `input_layout` / `pre_tokens` / `sparse_mode` 及同类标量），不要看 sources 写成了 `direct` 还是 `tensor_shape`。该列即使同时还是张量维，也抄 kwargs 名。禁止 id 空、禁止只把短名放 `candidate`。inspect 前必须回扫。
...
4. **仅 active 查 identifier。** 具名 kwargs **查 kwargs 名**（不要用 CSV 列名去碰尺寸字段）。没有 `TILING_FIELD` 也立刻把 `uo.id` 写成该 `call_args.name`，禁止空。尺寸列才查**列名**。dtype 列不要查 `*DType` / `Is*`。命中 `TILING_FIELD` 后，snippet 邻维可分给本路其它尺寸列（value 侧头维用主头维旁边带数字后缀的字段）。够闭合就停。
...
1. **标量 kwargs 列**（接到 `head_num`、`keep_prob`、`sparse_mode`、`pre_tokens`、`input_layout` 这类名字）：`uo.id` **等于** 该 `call_args.name`。禁止空。→ `confirmed`。不要改用同维 tiling 字段候选。
...
call_args:
  ...
  - name: head_num
    runtime_expr: n
    sources:
      - {column: N1, relation: direct}
mapping:
  B:
    control: {status: active}
    relation: tensor_shape
    confidence: confirmed
    uo: {id: b, candidate: ''}      # TILING_FIELD.name
  N1:
    control: {status: active}
    relation: direct
    confidence: confirmed
    uo: {id: head_num, candidate: ''}  # 抄标量 call_args.name，不要空、不要只放 candidate
  Dtype:
    control: {status: active}
