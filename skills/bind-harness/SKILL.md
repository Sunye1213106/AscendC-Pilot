---
name: bind-harness
description: 写出测试脚本的 golden/compare/精度性能入口草稿。tg-init 的 harness 切片使用。
---

# 绑定 harness

只写 `parts/harness.yaml`。本路回答「现有 runner 怎么跑、怎么比、精度/性能入口在哪、现在造得出什么」。不要写列 mapping，不要读另一路的 `bind.yaml`。正式 `init.yaml` 不是本步写的。

引擎不懂 runner。要打开入口脚本和 argparse，仓内若有用例设计 YAML（接口、dtype、range、精度/性能标准）一并打开。

## 为什么本路不写列

怎么跑、怎么比、现在造得出什么，是脚本事实。列对应哪个 UO 标识符是另一路的图事实。混在一份 YAML 里，精度口径会变成「按列名猜」，缺生成器会被 mapping 掩盖。本路写完应能单独回答：现有 runner 能否执行计划稍后会点名的精度/性能义务。

## 输入 / 输出 / 停

读：`repo_scan.yaml`。`kind=script_repo` 才读测试仓；`kind=default_input` 不要假装已有仓，把缺口写进 findings / generate_inputs，不要发明脚本路径。

点了脚本仓却扫不到目录 → 本切片失败，不是改写成 default_input。

完成：一份自洽的 harness 草稿，口径来自脚本事实。
...
4. **写 `modes.precision` / `modes.perf`。** argparse 同时有两种 mode 就分别写。默认值若是性能 mode，不得把默认当精度。设计文件分开写了精度标准与性能标准，照抄事实，不要发明阈值。有精度入口可读 `skills/precision-testing/SKILL.md`；有性能入口可读 `skills/performance-testing/SKILL.md`。
...
- [ ] golden / compare / modes / generate_inputs / findings 五块都有着落（无仓则明确缺失）
...
脚本仓怎么确认、scan 之后 Agent 必须打开什么：`references/test-script-repo.md`。init 易错点：`references/construction-gotchas.md`。Host HIT 与 harness oracle 的差别：`references/oracle.md`。
