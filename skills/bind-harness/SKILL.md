...
4. **写 `modes.precision` / `modes.perf`。** argparse 同时有两种 mode 就分别写。默认值若是性能 mode，不得把默认当精度。设计文件分开写了精度标准与性能标准，照抄事实，不要发明阈值。有精度入口可读 `skills/precision-testing/SKILL.md`；有性能入口可读 `skills/performance-testing/SKILL.md`。在 golden/modes 之外记下同一 `call.kind`（pta / aclnn / mixed）。
...
脚本仓怎么确认：`references/test-script-repo.md`。golden / compare / modes / generate_inputs：`references/harness-edge-cases.md`。
