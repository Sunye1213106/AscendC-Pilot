---
name: plan-fuse
description: 把测试意图融成有限覆盖计划。执行 /tg-plan 的 plan_fuse 时使用。
---
...
5. **精度 / 性能指针。** 义务含精度 → 读 `skills/precision-testing/SKILL.md`。harness 有性能入口或意图含性能 → 读 `skills/performance-testing/SKILL.md`。口径来自 init 的 harness mode，不是 Host HIT。
...
覆盖层与融合顺序：`references/plan-heuristics.md`。计划易错点：`references/planning-gotchas.md`。Planning Context 长什么样：`references/planning-context.md`。
