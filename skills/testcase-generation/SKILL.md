...
- 引理证明是 Round Analysis 的一部分；**禁止**“先搜完再统一证明”。
- `Replay reject ≠ 不可达`；只有完整源码证明通过后才能进 E。
...
Plan `level=L3` / `branch_outcome_coverage`：TD dump + `branch_eval` 增长 R；lemma 字段 pin 使对侧入 E。浅 writer 不得单独入 E。同样按轮分析，不把证明留到收尾。
