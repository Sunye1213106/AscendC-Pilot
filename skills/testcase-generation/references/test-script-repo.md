...
无已确认的测试脚本根
  ...
  → 意图里没有仓外绝对路径时，禁止主控把发现的仓内 tests/ 写入 `test_script_root` 代答
  → 用户原文或对话里已出现**算子仓外**绝对路径时写入 `test_script_root` / `pilot_run(test_script_root=…)`，不要塞 intent，不要再让子代理猜
...
已确认 --test-script-root
  → engine 扫描入口、argparse、表头与全表列画像（`tables[].profile`：type / n_unique / min/max / topk）
  → Agent 把列 mapping / 精度性能口径写进草稿；**domains 必须引用 profile，禁止通读 CSV**；promote 才落 init.yaml
...
2. 打开用例表。列值域以 scan `profile` 为准，不要通读几千行。每列 mapping：脚本读点（如 `get_case` / `CaseConfig.xxx`）+ UO 标识符。shape 列用 profile range；`dim_*` 用先无参数索引再 `Dim=Name`。
...
5. 多张 csv/xls 以 `tables[].profile` 为值域；用户没点名的表不当本次目标。
