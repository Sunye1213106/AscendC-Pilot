...
8. **`uo-query` 不是 Host Session Driver 工作流**（`host_driver=False`）：禁止 `pilot_run` / `acp start uo-query`。`host_driver=False` ≠ 没有 method bundle。简单查询主控直接调用 `pilot_cli` `uo-query`（stdout）；复杂查询委派 Task，主控综合。子代不得 Write `answer.yaml`。
