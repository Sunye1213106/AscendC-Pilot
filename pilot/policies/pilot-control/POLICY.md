...
3b. **`uo-query` 可见 LLM 路由**：用 operator-analysis skill 判断水平。先对人说出「自查 / 1 个 Task / N 路并行」及为什么，再动手。短问（一两跳）主控自己 `acp uo-query --mode`；深问同一轮 `Task(actor=uo-query)`（切片由主控分类，不以 Host `host_step.tasks` 为准）。**禁止** `pilot_run` / `acp start uo-query`。不要为「问题路由」空转子代理。子答 `UNKNOWN`/`PARTIAL` 不得抬成 high；禁止跨 architecture 证据闭合。
