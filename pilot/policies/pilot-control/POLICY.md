...
11b. **启动条件（Spec 权威）**：`requires_architecture=true` 的 workflow（`tg-init` / `tg-plan` / `tg-solve` / `uo-init` / `uo-update`）必须同时有 `--project` 与 `--architecture`。缺 arch 时控制面扫 `op_host|op_kernel/arch*` 返回 `ARCHITECTURE_REQUIRED` + `architecture_option_details`；AskQuestion 后执行一次 `acp start … --project … --architecture <选中>`。禁止编造仓内不存在的 arch，禁止静默默认 architecture。
