...
11b. **启动条件（Spec 权威）**：`requires_architecture=true` 的 workflow（`uo-init` / `uo-update`）必须同时有 `--project` 与 `--architecture`。缺 arch 时控制面扫 `op_host|op_kernel/arch*` 返回 `ARCHITECTURE_REQUIRED` + `architecture_option_details`；AskQuestion 后执行一次 `acp start … --project … --architecture <选中>`。`requires_uo_product=true` 的 workflow（`tg-*` / `ce-review` / `uo-query` / `uo-investigate`）以已有 `.uo` 为准：无产物 → `UO_PRODUCT_REQUIRED`（先 `/uo-init`）；架构从 `.uo` 继承，不另扫 arch* 当权威。禁止编造仓内不存在的 arch，禁止静默默认 architecture。
...
3a. **`uo-query` / `kb_lookup`（return_value）**：subagent **只**在最终消息返回一个 `kb-answer-v1` payload，**不得 Write `answer.yaml`**。Host adapter 优先把 Task return 直接交给 Runtime finalize；CLI/manual fallback 可用 `--result-file <kb-answer.yaml>`。`answer.yaml` 仅由 Runtime 物化并注入 identity。缺 `uo/checks/integrity.yaml` 从来不是 kb_lookup 的修复目标；那是 uo-init verify 收据。
