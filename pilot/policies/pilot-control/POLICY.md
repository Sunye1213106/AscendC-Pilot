...
9. **Subagent / primary_interactive 派发**：仅派发 Spec 中声明的 actor；Primary 禁止代写正式 IR。Task 须带 `subagent_type`/`agent`=actor 与 `action_id`。**Task 正文只能原样使用 prepare 返回的 `task_prompt_stub`**（禁止复述 METHOD、禁止塞额外目标、禁止空 prompt）。**`uo-query` 若 `host_step.tasks` ≥2：同一轮并行派发每条 `tasks[i].task_prompt_stub`，全部返回后 Primary 按各 Task 原生全文综合（一张 ticket）**；禁止只转述某一个子代理，禁止发明子代理没引用的事实。**同 Action rework 必须 resume 原 Task session**。正式产物仅 Host **finalize**。已删除的历史 Action（如 `extract_plan` / `uo-semantic-resolve` / `adjudicate_llm_tasks` 及历史 UO scope 人工确认类 Action）不得再派发。
...
3b. **`uo-query` 可见 LLM 路由**：用 operator-analysis skill 判断水平。先对人说出「自查 / 1 个 Task / N 路并行」及为什么，再动手。短问（一两跳）主控自己 `acp uo-query --mode`；深问（METHOD≥2行 / 多个独立结案条件）同一轮 `Task(actor=uo-query)`，**禁止**降成主控连查。切片看 METHOD 行数，不看症状是否相关：相关 ≠ 单域。禁止「一条因果链 / 一个 agent 更连贯」把 ≥2 行收成 1 路。禁止把整题丢给一个子代理。每个 Task 只带本片 FOCUS + `FIRST_QUERY: acp uo-query --mode <本片唯一先查> --project <绝对路径>` + 本片那一句。切片由主控分类，不以 Host `host_step.tasks` 为准。**禁止** `pilot_run` / `acp start uo-query`。不要为「问题路由」空转子代理。子答 `UNKNOWN`/`PARTIAL` 不得抬成 high；禁止跨 architecture 证据闭合。未闭合再开一轮 Task，不要问「要不要继续」。
