...
8. bash 优先用工具 `workdir` 指向算子目录；若写 `cd <dir> && acp …`，Pilot 只认末尾纯 `acp` 段（禁止夹杂其它命令）。禁止用 bash/`>`/`Set-Content`/`tee` 写入 `.ascendc-pilot/**` 正式产物以绕过 Write 围栏。**只读定位**允许：`ls`/`Get-ChildItem`/`grep`/`rg`/`Select-String`/`findstr`（无写重定向）；仍不得当高置信证据。优先 `pilot_run`。若必须 bash 调 `acp`，禁止管道缓冲：PowerShell `| Select-Object -Last N`、`| Out-String`，bash `| tail`。
9. **Subagent / primary_interactive 派发**：仅派发 Spec 中声明的 actor；Primary 禁止代写正式 IR。Task 须带 `subagent_type`/`agent`=actor 与 `action_id`。**Task 正文只能原样使用 prepare 返回的 `task_prompt_stub`**（禁止复述 METHOD、禁止塞额外目标、禁止空 prompt）。**同 Action rework 必须 resume 原 Task session**。正式产物仅 Host **finalize**。已删除的历史 Action（如 `extract_plan` / `uo-semantic-resolve` / `adjudicate_llm_tasks` 及历史 UO scope 人工确认类 Action）不得再派发。
...
11b. **启动条件（Spec 权威）**：`requires_architecture=true` 的 workflow（`uo-init` / `uo-update`）必须同时有 `--project` 与 `--architecture`。缺 arch 时：**先**跑 `acp scan-architectures --project <算子目录>`，阅读返回的 `layout` / `architecture_option_details`，再 AskQuestion（选项原样使用 details）；也可直接 `acp start … --project …`（无 `--architecture`）拿 `ARCHITECTURE_REQUIRED` + 同结构选项。AskQuestion 后执行一次 `acp start … --project … --architecture <选中>`。**禁止**仓根 Glob `arch*`、翻 `cmake/` / `classify_rule.yaml` / `get_soc_version.py` 考古猜 arch。`requires_uo_product=true` 的 workflow（`tg-*` / `ce-review` / `uo-query` / `uo-investigate`）以已有 `.uo` 为准：无产物 → `UO_PRODUCT_REQUIRED`（先 `/uo-init`）；架构从 `.uo` 继承，不另扫 arch* 当权威。禁止编造仓内不存在的 arch，禁止静默默认 architecture。
...
11d. **Clang 探针 / CANN include 失败**：`CANN_ENV_NOT_READY`、`clang_probe_unclean`、探针 `file not found` 时，这是 CANN 包 / BuildContext 问题。`uo-init` prepare 的 **include-heal** 会在解包树定位头文件并补 runtime `-I`（写入 extras yaml，不改共享 `build_context.yaml`）。Agent **不要**把缺头文件当成算子 `unknown`，**不要**用 `UO_TEST_ALLOW_UNVERIFIED_SCOPE`。仅当 `include_heal.unresolved` 非空才对照官方编译文件改 `engines/understand-operator/spec/build_context.yaml` 并清 probe 缓存重试。细则：`skills/operator-analysis/references/codemap-build-gotchas.md`。
11e. **人话出口**：凡 `needs_human_decision` / 阶段总结 / Goal 推进 / 失败给人看的摘要，必须意图+动作+后果（见 `human-voice-invariants.md`）。禁止把 referee 黑话贴给用户。
11f. **User Goal（全量 case）**：匹配「全量/全覆盖/tilingkey case/建立 TilingKey 全覆盖测试」→ `control/user_goal.yaml` + `acp start … --intent <短语>`；`complete` 后若有 `recommended_next_workflow` 则人话说明并 start 下一步（init→plan→solve），不要只跑完 tg-init 就停。
...
3a. **`uo-query` / `kb_lookup`（return_value）**：subagent **只**在最终消息返回一个 `kb-answer-v1` payload，**不得 Write `answer.yaml` / scratch**。Host adapter（如 OpenCode return_value 插件）捕获 Task 末尾 payload 后，Primary 优先无文件执行 `acp run-action kb_lookup --finalize`；仅当插件/环境未注入时才用 `--result-file` fallback。禁止 Primary 为 finalize 手写 scratch yaml。`answer.yaml` 仅由 Runtime 物化并注入 identity。缺 `uo/checks/integrity.yaml` 从来不是 kb_lookup 的修复目标；那是 uo-init verify 收据。
3b. **复杂多 claim 查询编排**：Primary 按复杂度拆 Host / Kernel / Perf 等窄 claim，并行多个 `Task(actor=uo-query)`（共用 UO Product Handle；禁止嵌套第二个 `/uo-query` lifecycle），再合成一条确切答案。子答 `UNKNOWN`/`PARTIAL` 不得被抬成 high；禁止跨 architecture 证据闭合。
