...
8. bash 优先用工具 `workdir` 指向算子目录；若写 `cd <dir> && acp …`，Pilot 只认末尾纯 `acp` 段（禁止夹杂其它命令）。禁止用 bash/`>`/`Set-Content`/`tee` 写入 `.ascendc-pilot/**` 正式产物以绕过 Write 围栏。**只读定位**允许：`ls`/`Get-ChildItem`/`grep`/`rg`/`Select-String`/`findstr`（无写重定向）；仍不得当高置信证据。**禁止**对 `acp` 使用会缓冲全部输出的管道：PowerShell `| Select-Object -Last N`、`| Out-String`，bash `| tail`/`| tail -n`。必须直接跑 `acp … --project …`，让 stderr 实时可见（`[acp-auto]` / `[uo]` 心跳与步骤）；需要截断时用工具 UI 折叠，勿在 shell 里 `-Last`。
...
11d. **Clang 探针 / CANN include 失败**：`CANN_ENV_NOT_READY`、`clang_probe_unclean`、探针 `file not found` 时，先 `acp doctor`；再对照算子仓官方编译文件（`build.sh`、`CMakeLists.txt` include）修正 Pilot `engines/understand-operator/spec/build_context.yaml`，清 probe 缓存后重试 prepare。细则：`skills/operator-analysis/references/codemap-build-gotchas.md`。
