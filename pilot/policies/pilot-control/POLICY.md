...
4. Gate fail ≠ 立即 `blocked`；保持 phase，进入 `rework_required` / `human_required`。进入 `human_required` 后必须弹出可点选框，禁止只写旁白。
...
8. **`uo-query` 不是 Host Session Driver 工作流**（`host_driver=False`）：禁止 `pilot_run` / `acp start uo-query`。`host_driver=False` ≠ 没有 method bundle。短问主控 `acp uo-query --mode`（stdout）；深问 prepare + Task + Primary finalize。子代不得 Write `answer.yaml`。
9. 关键参数不明确 → 立刻 AskQuestion。`requires_architecture=true` 必须同时有 `--project` 与 `--architecture`。禁止静默默认 architecture，禁止仓根考古猜 arch。
...
12. 人话出口：`needs_human_decision` / 阶段总结必须意图+动作+后果。禁止把 referee 黑话贴给用户。
