...
1. 加载匹配的 workflow skill → `acp start`（`uo-query` 短问除外：主控自己 `acp uo-query --mode`）。若返回 `needs_human_decision`：用 `question`/AskQuestion 可点选框 → `--decision continue|reinit` → 立刻 `todowrite`（全量）
...
3b. **`uo-query` 路由在主控**：用 operator-analysis skill 大体判断查什么。短问（一两跳）主控自己 `acp uo-query --mode` 作答；深问再 `Task(actor=uo-query)` 或启动 `/uo-query`。不要为「问题路由」空转子代理。子答 `UNKNOWN`/`PARTIAL` 不得抬成 high；禁止跨 architecture 证据闭合。
3c. Task 正文只用 prepare 返回的 `task_prompt_stub`；用户问题已由控制面注入 stub / `prompt.md` 的 `USER QUESTION` / `## User question`。Primary **禁止**在 stub 外另塞长篇分析目标。
