...
3. Grep / rg / `Select-String` **只用于定位**（OpenCode Grep 或 bash 只读搜索均允许），不可单独作为复杂语义结论 / high / `source_verified` 的唯一证据。Windows `findstr` 路径须用反斜杠（`D:\…\file`）；正斜杠 `D:/…` 会被当成开关导致「无法打开」。**`uo-query` / `kb_lookup` 禁止仓级 grep/findstr/rg**；用 `acp uo-query`，最后才 `acp ro-search --paths <已 citation 文件>`。
