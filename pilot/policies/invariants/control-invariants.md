...
3. `/uo-init` `/uo-update` 缺 architecture → AskQuestion，禁止猜测。第一轮 `workflow=auto` 省略 architecture。隔离 clone 走 `auto`。意图未给出仓外测试路径或 git URL 时 `/tg-init` 由 Host 问三项；已给出则写入 `test_script_root`，不要再问。禁止把仓内 `tests/` 填进 `test_script_root`。
