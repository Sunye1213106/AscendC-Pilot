...
2. 工作流只用 `pilot_run`。禁止对 `uo-query` 调用 `pilot_run`（简单/复杂查询见 `routing/uo-query.md`）。`host_step.tasks`≥2 时按 `task_prompt_stub` 原样派发。非 primary 不得再派发 Task。`primary_review` 下一发仅 `PASS` 或 `REWORK bind` / `REWORK harness,bind`。禁止 `--help`。禁止 `force_new` 除非用户明确删除重开。
