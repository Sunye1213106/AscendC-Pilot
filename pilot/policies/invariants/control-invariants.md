...
5. When `host_step.kind=done` for uo-init/update: Read `host_step.quality_path` (`.ascendc-pilot/<arch>/uo/checks/quality.yaml`; never the unscoped `.ascendc-pilot/uo/` tree) and tell the user node/relation counts plus unclosed buckets and why. For uo-query: 短问 speak `acp uo-query` stdout; 深问 speak the native Task return then Primary finalize. MUST NOT Write `answer.yaml` (children). Do not paste Todo/stage panels or compress the child's answer into YAML. If any child is PARTIAL / 未闭合 / contradicts another / skipped CodeMap, Primary **must** launch another same-conversation round of Task(`agent=uo-query`) (FOCUS=the named gap) before closing. Do not ask 要不要继续.
...
# - uo-query is NOT driver-owned: Primary visible classify → 短问 DIY / 深问 Task + Primary finalize
