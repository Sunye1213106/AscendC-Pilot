...
3. Missing required params (`--project`, `--architecture`, resume decision) → AskQuestion immediately with **verbatim** options; do not guess architecture. Exception: `workflow=auto` may start without `--project` when the Intent LLM later yields an allowlisted PR SourceRef (Workspace Manager acquires the tree). If the current user message already names a **unique legal** option (one on-disk `arch*`), use it and do not AskQuestion; echo the choice. If the user interrupts or replies in chat instead of clicking, the latest message wins: map it onto an existing option or supersede the pending confirmation (`interpret-user-turn`). Do not re-ask. Do not treat interrupt as wipe/reinit. Natural-language Intake is LLM-first: do not phrase-route or pre-extract URLs before Intent.
