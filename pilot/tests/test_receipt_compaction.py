...
def test_issue_receipt_stores_slim_checker(tmp_path: Path) -> None:
    ...
    assert verify_receipt(tmp_path, action_id="resolve").get("ok") is True
...
def test_recommend_next_uses_one_done_check_per_action(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(pipe_mod, "preferred_pipeline", lambda *args, **kwargs: ["resolve", "apply_gap_patch"])
    monkeypatch.setattr(pipe_mod, "_not_applicable_proof", lambda *args, **kwargs: False)
    allowed = [
        {"id": "resolve", "label_zh": "Resolve"},
        {"id": "apply_gap_patch", "label_zh": "Merge"},
    ]
    ...
    assert rec and rec["id"] == "resolve"
    assert calls == ["resolve", "apply_gap_patch"]
    assert rec["missing_actions"] == ["resolve", "apply_gap_patch"]
...
def test_verify_compacts_legacy_fat_receipt_and_resigns(tmp_path: Path) -> None:
    ...
    payload = {
        "identity": f"{run_id}:resolve:uo-semantic-resolver",
        "run_id": run_id,
        "workflow_id": "uo-init",
        "phase": "resolve",
        "actor_type": "producer",
    ...
    path = run_dir(tmp_path, run_id) / "subagents" / f"{run_id}_resolve_uo-semantic-resolver.yaml"
