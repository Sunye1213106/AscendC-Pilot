...
def test_router_slash_only_no_nl_keywords():
    """Generic NL is not keyword-routed; caller should use workflow=auto."""
    ...
    assert nl.get("error") == "use_auto"
    assert nl.get("workflow_id") == "auto"
    assert "uo-init" in (nl.get("candidates") or [])
    assert nl.get("message_zh")
    assert route("帮我建库初始化知识库").get("ok") is False
    assert route("完全无关的话").get("error") == "use_auto"
    assert route("/uo-diff").get("ok") is False
    goal = route("建立 TilingKey 全覆盖测试")
    assert goal.get("ok") is False and goal.get("error") == "use_auto"
    ce = route("验证这次改动")
    assert ce.get("ok") is False and ce.get("workflow_id") == "auto"
