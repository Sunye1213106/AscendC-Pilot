...
def _build(
    *,
    ...
    "tg-plan": {
        ...
        "retry_budget": 3,
        "states": [
            _st("prepare", "校验 init.yaml 并写出改动包"),
            _st("model", "Plan Owner 交 Coverage IR"),
            _st("validate", "写入并校验 plan.md"),
            _st("approve", "批准规划"),
        ],
        "transitions": [
            _tr("prepare", "model"),
            ...
            _tr("validate", "model", kind="rework", reason_codes=["PLAN_INVALID", "PLAN_INGEST_REQUIRED"]),
            _tr("approve", "model", kind="rework", reason_codes=["rework"]),
        ],
        "phase_gates": {
            "prepare": ["tg_init_confirmed", "kb_fingerprint_fresh"],
            "approve": ["plan_approved"],
        },
        "complete_gates": ["plan_approved"],
        "pipelines": {
            "prepare": ["plan_precheck"],
            "model": ["plan_ingest"],
            "validate": ["plan_promote", "plan_validate"],
        ...
        "actions": [
            ...
            _act(
                "plan_ingest",
                label_zh="主控派 Plan Owner 并交回 Coverage IR",
                phases=["model"],
                workflow_id="tg-plan",
                agent_id="ascendc-pilot",
                role_id="controller",
                ...
                capability_ids=[],
                skill_id="test-plan",
                refs=["coverage-ir.md", "target-planning.md", "evidence.md"],
                knowledge_refs=[
                    "ascendc/precision.md",
                    "ascendc/performance.md",
                ],
                task_prompt_id="tg/plan-owner",
            ...
            _act(
                "plan_validate",
                label_zh="校验 Target / Dimension / Guard / coverage",
                phases=["validate"],
                workflow_id="tg-plan",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
        ...
        "phases": ["prepare", "model", "validate", "approve"],
    ...
    "tg-solve": {
        ...
        "states": [
            _st("gate", "校验已批准规划"),
            _st("construct", "为 OPEN 义务构造输入"),
            _st("replay", "Host 回放"),
            _st("analyze", "只处理 MISS / UNKNOWN"),
            _st("certify", "ledger 闭合则签发"),
        ],
        "transitions": [
            _tr("gate", "construct"),
        ...
        "pipelines": {
            "gate": ["solve_precheck", "compile_obligations"],
            "construct": ["construct_cases", "construct_promote"],
            "replay": ["replay_round", "coverage_eval"],
            "analyze": [
                "analyze_round",
                "analyze_promote",
                "source_proof",
        ...
        "actions": [
            ...
            _act(
                "compile_obligations",
                label_zh="从 plan.md 展开覆盖义务并写入 worklog 围栏",
                phases=["gate"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
            ...
            _act(
                "construct_cases",
                label_zh="为 OPEN 义务交回 seed fill",
                phases=["construct"],
                workflow_id="tg-solve",
                agent_id="tg-analyst",
                role_id="producer",
                ...
                output_contract_id="tg-construct-v1",
                output_mode="return_value",
                allowed_write_paths=[],
                produces=[],
            ),
            _act(
                "construct_promote",
                label_zh="把 candidate / recipe 收进 session 与 replay 缓存",
                phases=["construct"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                output_contract_id="tg-construct-capture-v1",
                produces=[],
            ),
            _act(
                "replay_round",
            ...
            _act(
                "coverage_eval",
                label_zh="确定性分类义务并更新 worklog 围栏",
                phases=["replay"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
            ...
            _act(
                ...
                capability_ids=["kb-query", "source-navigation", "source-reading"],
                skill_id="solve",
                method_ref="replay-classification.md",
                task_prompt_id="tg/analyze-round",
                output_contract_id="tg-analyze-v1",
                output_mode="return_value",
                allowed_write_paths=[],
                produces=[],
            ),
            ...
            _act(
                "source_proof",
                label_zh="为 proof_request 写 atomic 证书",
                phases=["analyze"],
                workflow_id="tg-solve",
                agent_id="tg-analyst",
                role_id="producer",
                ...
                refs=["proof-certificate.md", "proof-obligations.md"],
                task_prompt_id="tg/source-proof",
                output_contract_id="tg-source-proof-v1",
                output_mode="return_value",
                allowed_write_paths=[],
                produces=[],
            ),
            _act(
                ...
                task_prompt_id="tg/proof-review",
                output_contract_id="tg-proof-review-v1",
                output_mode="return_value",
                allowed_write_paths=[],
                produces=[],
            ),
            _act(
                "proof_promote",
                label_zh="accept 后写入 PROVED_UNREACHABLE",
                phases=["analyze"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
            ...
            _act(
                ...
                produces=["tg/cases.csv", "tg/cases.xls", "tg/cases.xlsx"],
            ),
        ],
        "agents": [
            {"id": "deterministic-tg-engine", "role": "deterministic_engine"},
