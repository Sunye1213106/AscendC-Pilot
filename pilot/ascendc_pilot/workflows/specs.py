"""Concrete workflow specs (English ids + Chinese labels)."""

from __future__ import annotations

from typing import Any

# Shared stable policies composed into every semantic / engine action.
DEFAULT_POLICY_IDS: list[str] = [
    "source-authority",
    "code-access",
    "evidence",
    "language",
    "pilot-control",
    "output-quality",
]


def _st(sid: str, label_zh: str) -> dict[str, str]:
    return {"id": sid, "label_zh": label_zh}


def _tr(frm: str, to: str, *, kind: str = "forward", reason_codes: list[str] | None = None) -> dict[str, Any]:
    row: dict[str, Any] = {"from": frm, "to": to, "kind": kind}
    if reason_codes:
        row["reason_codes"] = reason_codes
    return row


def _act(
    action_id: str,
    *,
    label_zh: str,
    phases: list[str],
    workflow_id: str,
    checker_required: bool = True,
    referee_required: bool = False,
    gates: list[str] | None = None,
    agent_id: str | None = None,
    role_id: str | None = None,
    execution_mode: str | None = None,
    policy_ids: list[str] | None = None,
    capability_ids: list[str] | None = None,
    action_method_id: str | None = None,
    task_prompt_id: str | None = None,
    context_profile_id: str | None = None,
    output_contract_id: str | None = None,
    output_mode: str | None = None,
    staging_contract_id: str | None = None,
    merge_action_id: str | None = None,
    allowed_write_paths: list[str] | None = None,
    allowed_read_paths: list[str] | None = None,
    forbidden_write_paths: list[str] | None = None,
    forbidden_read_paths: list[str] | None = None,
) -> dict[str, Any]:
    """Declare a Pilot Action with compositional references.

    ``actors`` is derived from ``agent_id`` for authorize / spawn checks.
    Workflow Spec is the sole editable authority for identity fields.
    """
    from ascendc_pilot.ownership import (
        ACTION_FORBIDDEN_READ_PATHS,
        ACTION_FINALIZER_WRITE_PATHS,
        ACTION_PRODUCER_WRITE_PATHS,
        ACTION_READ_PATHS,
        ACTION_WRITE_PATHS,
        infer_execution_mode,
    )

    method_id = action_method_id or f"{workflow_id}/{action_id.replace('_', '-')}"
    actors = [agent_id] if agent_id else []
    mode = infer_execution_mode(
        agent_id=agent_id,
        role_id=role_id,
        execution_mode=execution_mode,
    )
    writes = allowed_write_paths
    if writes is None:
        if output_mode == "staged" and role_id == "producer":
            writes = list((ACTION_PRODUCER_WRITE_PATHS.get(workflow_id) or {}).get(action_id) or [])
        if not writes:
            writes = list((ACTION_WRITE_PATHS.get(workflow_id) or {}).get(action_id) or [])
    reads = allowed_read_paths
    if reads is None:
        reads = list((ACTION_READ_PATHS.get(workflow_id) or {}).get(action_id) or [])
    forbid_reads = forbidden_read_paths
    if forbid_reads is None:
        forbid_reads = list((ACTION_FORBIDDEN_READ_PATHS.get(workflow_id) or {}).get(action_id) or [])
    # Staged producers must not write finalizer canonical paths.
    forbid_writes = list(forbidden_write_paths or [])
    if output_mode == "staged" and role_id == "producer":
        for p in (ACTION_FINALIZER_WRITE_PATHS.get(workflow_id) or {}).get(action_id) or []:
            if p not in forbid_writes:
                forbid_writes.append(p)
    row: dict[str, Any] = {
        "id": action_id,
        "label_zh": label_zh,
        "phases": list(phases),
        "checker_required": checker_required,
        "referee_required": referee_required,
        "gates": list(gates or []),
        "agent_id": agent_id,
        "role_id": role_id,
        "execution_mode": mode,
        "policy_ids": list(policy_ids if policy_ids is not None else DEFAULT_POLICY_IDS),
        "capability_ids": list(capability_ids or []),
        "action_method_id": method_id,
        "task_prompt_id": task_prompt_id,
        "context_profile_id": context_profile_id or f"{workflow_id}-{action_id.replace('_', '-')}",
        "output_contract_id": output_contract_id,
        "allowed_write_paths": list(writes or []),
        "allowed_read_paths": list(reads or []),
        "forbidden_write_paths": list(forbid_writes),
        "forbidden_read_paths": list(forbid_reads or []),
        # Derived for authorize / Task spawn (single primary actor).
        "actors": actors,
    }
    if output_mode:
        row["output_mode"] = output_mode
    if staging_contract_id:
        row["staging_contract_id"] = staging_contract_id
    if merge_action_id:
        row["merge_action_id"] = merge_action_id
    return row


_UO_INIT_RESOLVE_GATES = [
    "key_triage_required",
    "key_resolve_receipt",
    "empty_only_producer",
    "confidence_gate",
    "key_report_quality",
    "confidence_closed_high",
    "confidence_reason_review",
]

# Static obligation id → gate id that settles it when that gate passes.
STATIC_OBLIGATION_GATE_MAP: dict[str, str] = {
    "scope_confirmed": "scope_receipt",
    "kb_integrity_passed": "integrity",
    "kb_review_passed": "kb_review",
    "kb_ready": "kb_ready",
    "uo_ready": "uo_ready",
    "init_confirmed": "init_confirmed",
    "plan_approved": "plan_approved",
    "solve_terminal": "solve_terminal",
}

CLOSED_OBLIGATION_STATUSES = frozenset(
    {
        "resolved",
        "verified",
        "not_applicable",
        "human_required",
        "rejected",
    }
)

_CAPS_CODE = ["source-reading", "cbm-navigation", "kb-query"]
_CAPS_EXTRACT = [
    "source-reading",
    "cbm-navigation",
    "kb-query",
    "semantic-resolution",
    "structured-ir-query",
    "readonly-source-search",
    "action-scratch",
    "sharded-llm-producer",
    "bounded-semantic-batch",
    "producer-self-check",
]
_CAPS_ADJUDICATE = [
    "source-reading",
    "cbm-navigation",
    "kb-query",
    "semantic-resolution",
    "structured-ir-query",
    "readonly-source-search",
    "action-scratch",
    "sharded-semantic-producer",
    "sharded-llm-producer",
    "bounded-semantic-batch",
    "producer-self-check",
]
_CAPS_RESOLVE = [
    "source-reading",
    "cbm-navigation",
    "kb-query",
    "semantic-resolution",
    "action-scratch",
    "sharded-llm-producer",
    "bounded-semantic-batch",
    "producer-self-check",
]
_CAPS_REVIEW = ["structured-review", "kb-query"]
_CAPS_CONTRACT = ["contract-building", "kb-query", "obligation-analysis"]
_CAPS_OBLIGATION = ["obligation-analysis", "kb-query"]

WORKFLOWS: dict[str, dict[str, Any]] = {
    "uo-init": {
        "slash": "/uo-init",
        "engine": "uo",
        "entry_state": "prepare",
        "terminal_ready_states": ["review"],
        "retry_budget": 3,
        "states": [
            _st("prepare", "环境准备"),
            _st("scope", "范围确认"),
            _st("extract", "结构抽取"),
            _st("normalize", "归一化"),
            _st("export", "导出与校验"),
            _st("review", "产物审查"),
        ],
        "transitions": [
            _tr("prepare", "scope"),
            _tr("scope", "extract"),
            _tr("extract", "normalize"),
            _tr("normalize", "export"),
            _tr("export", "review"),
            _tr("extract", "scope", kind="rework", reason_codes=["SCOPE_REWORK", "SCOPE_FAILED"]),
            _tr("normalize", "extract", kind="rework", reason_codes=["EXTRACT_REWORK"]),
            _tr("export", "normalize", kind="rework", reason_codes=["INTEGRITY_REWORK", "GAP_REWORK"]),
            _tr("review", "normalize", kind="rework", reason_codes=["KB_REVIEW_REWORK"]),
            _tr("review", "export", kind="rework", reason_codes=["export_kb", "integrity"]),
            _tr("review", "scope", kind="rework", reason_codes=["scope", "SCOPE_REWORK"]),
        ],
        "phase_gates": {
            "prepare": ["layout_receipt"],
            "scope": ["scope_probe_clean", "scope_receipt"],
            "extract": ["extract_receipt"],
            "normalize": ["normalize_receipt"],
            "export": ["integrity"],
            "review": ["kb_review"],
        },
        "pipelines": {
            "prepare": ["prepare_layout"],
            "scope": ["scope_scan", "scope_confirm"],
            "extract": [
                "extract_host",
                "extract_tiling_key",
                "extract_registry",
                "extract_kernel",
            ],
            "normalize": [
                "normalize_variables",
                "derive_key_fields",
                "normalize_predicates",
                "resolve_gaps",
                "apply_gap_patch",
            ],
            "export": ["export_kb", "build_index", "export_integrity"],
            "review": ["kb_review"],
        },
        "complete_gates": [
            "scope_receipt",
            "extract_receipt",
            "integrity",
            "kb_review",
        ],
        "meta": {
            "max_gap_resolve_attempts": 3,
            "recovery_by_reason": {
                "SCOPE_REWORK": {
                    "type": "transition",
                    "target_phase": "scope",
                    "next_action": "scope_confirm",
                },
                "EXTRACT_REWORK": {"type": "action", "action_id": "extract_host"},
                "GAP_REWORK": {"type": "action", "action_id": "resolve_gaps"},
                "INTEGRITY_REWORK": {"type": "action", "action_id": "export_integrity"},
                "KB_REVIEW_REWORK": {"type": "action", "action_id": "kb_review"},
            },
        },
        "actions": [
            _act(
                "prepare_layout",
                label_zh="创建知识库目录",
                phases=["prepare"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["layout_receipt"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="kb-layout-v1",
            ),
            _act(
                "scope_scan",
                label_zh="范围扫描与探针",
                phases=["scope"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["scope_probe_clean"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="scope-candidates-v1",
            ),
            _act(
                "scope_confirm",
                label_zh="确认分析范围",
                phases=["scope"],
                workflow_id="uo-init",
                agent_id="ascendc-pilot",
                role_id="controller",
                execution_mode="primary_interactive",
                gates=["scope_receipt"],
                capability_ids=["cbm-navigation", "source-reading"],
                task_prompt_id="uo/scope-confirmation",
                output_contract_id="scope-confirmed-v1",
            ),
            _act(
                "extract_host",
                label_zh="抽取 Host IR",
                phases=["extract"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="extract-host-v1",
            ),
            _act(
                "extract_tiling_key",
                label_zh="抽取 TilingKey 绑定",
                phases=["extract"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="extract-tiling-key-v1",
            ),
            _act(
                "extract_registry",
                label_zh="抽取 Registry 竞价",
                phases=["extract"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="extract-registry-v1",
            ),
            _act(
                "extract_kernel",
                label_zh="抽取 Kernel 折叠分支",
                phases=["extract"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["extract_receipt"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="extract-kernel-v1",
            ),
            _act(
                "normalize_variables",
                label_zh="变量归一化",
                phases=["normalize"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="normalize-variables-v1",
            ),
            _act(
                "derive_key_fields",
                label_zh="派生 TilingKey 字段",
                phases=["normalize"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="derive-key-fields-v1",
            ),
            _act(
                "normalize_predicates",
                label_zh="谓词归一化",
                phases=["normalize"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="normalize-predicates-v1",
            ),
            _act(
                "resolve_gaps",
                label_zh="闭合残余 blocker",
                phases=["normalize"],
                workflow_id="uo-init",
                agent_id="uo-gap-resolve",
                role_id="producer",
                execution_mode="subagent",
                gates=["gap_patch_evidence"],
                capability_ids=_CAPS_RESOLVE,
                task_prompt_id="uo/resolve-gaps",
                output_contract_id="resolve-gaps-v1",
                output_mode="staged",
                staging_contract_id="resolve-gaps-staging-v1",
                merge_action_id="apply_gap_patch",
                allowed_write_paths=[
                    "runs/{run_id}/actions/resolve_gaps/parts/**",
                    "runs/{run_id}/actions/resolve_gaps/scratch/**",
                    "runs/{run_id}/actions/resolve_gaps/staging.yaml",
                ],
            ),
            _act(
                "apply_gap_patch",
                label_zh="应用 gap patch",
                phases=["normalize"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["normalize_receipt"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="gap-patch-v1",
            ),
            _act(
                "export_kb",
                label_zh="导出分层 KB",
                phases=["export"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="export-kb-v1",
            ),
            _act(
                "build_index",
                label_zh="重建 SQLite 索引",
                phases=["export"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="build-index-v1",
            ),
            _act(
                "export_integrity",
                label_zh="完整性校验",
                phases=["export"],
                workflow_id="uo-init",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["integrity"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="integrity-v1",
            ),
            _act(
                "kb_review",
                label_zh="KB 产物审查",
                phases=["review"],
                workflow_id="uo-init",
                agent_id="uo-kb-review",
                role_id="referee",
                referee_required=True,
                gates=["kb_review"],
                capability_ids=_CAPS_REVIEW,
                task_prompt_id="uo/kb-review",
                output_contract_id="kb-review-v1",
            ),
        ],
        "agents": [
            {"id": "uo-gap-resolve", "role": "producer"},
            {"id": "uo-kb-review", "role": "referee"},
            {"id": "deterministic-uo-engine", "role": "deterministic_engine"},
        ],
        "static_obligations": [
            {"id": "scope_confirmed", "label_zh": "范围已确认"},
            {"id": "kb_integrity_passed", "label_zh": "完整性通过"},
            {"id": "kb_review_passed", "label_zh": "产物审查通过"},
        ],
        "dynamic_obligation_sources": ["ir/unresolved.yaml"],
        "write_roots": ["uo", "runs", "state", "context"],
        "reset_policy": {
            "reinit_delete": ["uo"],
            "reinit_preserve": [],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["prepare", "scope", "extract", "normalize", "export", "review"],
        "gates": [
            "layout_receipt",
            "scope_probe_clean",
            "scope_receipt",
            "extract_receipt",
            "normalize_receipt",
            "gap_patch_evidence",
            "integrity",
            "kb_review",
        ],
    },
    "uo-update": {
        "slash": "/uo-update",
        "engine": "uo",
        "entry_state": "detect",
        "terminal_ready_states": ["diff"],
        "retry_budget": 3,
        "states": [
            _st("detect", "变更检测"),
            _st("plan", "更新计划"),
            _st("apply", "应用变更"),
            _st("resolve", "语义闭合"),
            _st("export", "导出与校验"),
            _st("diff", "差异摘要"),
        ],
        "transitions": [
            _tr("detect", "plan"),
            _tr("plan", "apply"),
            _tr("apply", "resolve"),
            _tr("resolve", "export"),
            _tr("export", "diff"),
            _tr("resolve", "resolve", kind="rework", reason_codes=["KEY_REWORK", "CONFIDENCE_REWORK"]),
            _tr("export", "resolve", kind="rework", reason_codes=["INTEGRITY_REWORK", "residual_resolve", "input_derivable"]),
            _tr(
                "export",
                "apply",
                kind="rework",
                reason_codes=["EXTRACT_REWORK", "entrypoints", "extract_plan"],
            ),
            _tr("detect", "diff", kind="forward", reason_codes=["DIFF_ONLY"]),
        ],
        "phase_gates": {
            # key_triage LLM chain deferred on new KB — stubs + confidence/integrity only
            "resolve": ["confidence_gate"],
            "export": ["integrity"],
        },
        "pipelines": {
            "detect": ["detect_changes"],
            "plan": ["plan_update"],
            "apply": ["apply_update"],
            "resolve": [
                "key_triage",
                "key_resolution",
                "confidence_report",
                "confidence_review",
            ],
            "export": ["export_integrity"],
            "diff": ["diff_summary"],
        },
        "complete_gates": [
            "confidence_gate",
            "integrity",
        ],
        "complete_gates_diff_only": [],
        "actions": [
            _act(
                "detect_changes",
                label_zh="检测源码变更",
                phases=["detect"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=["source-reading"],
                output_contract_id="change-detect-v1",
            ),
            _act(
                "plan_update",
                label_zh="制定更新计划",
                phases=["plan"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=["source-reading", "kb-query"],
                output_contract_id="update-plan-v1",
            ),
            _act(
                "apply_update",
                label_zh="应用变更",
                phases=["apply"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=["source-reading", "kb-query"],
                output_contract_id="update-apply-v1",
            ),
            _act(
                "key_triage",
                label_zh="KEY 粗分（新 KB stub）",
                phases=["resolve"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                action_method_id="uo-update/key-triage",
                task_prompt_id=None,
                output_contract_id="key-triage-v1",
            ),
            _act(
                "key_resolution",
                label_zh="KEY 语义闭合（新 KB stub）",
                phases=["resolve"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                action_method_id="uo-update/key-resolution",
                task_prompt_id=None,
                output_contract_id="input-derivable-patch-v1",
            ),
            _act(
                "confidence_report",
                label_zh="生成置信度报告",
                phases=["resolve"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["confidence_gate"],
                capability_ids=[],
                output_contract_id="confidence-report-v1",
            ),
            _act(
                "confidence_review",
                label_zh="置信度原因审查（新 KB stub）",
                phases=["resolve"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=[],
                action_method_id="uo-update/confidence-review",
                task_prompt_id=None,
                output_contract_id="confidence-reason-review-v1",
            ),
            _act(
                "export_integrity",
                label_zh="导出与完整性校验",
                phases=["export"],
                workflow_id="uo-update",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["integrity"],
                capability_ids=[],
                output_contract_id="integrity-v1",
            ),
            _act(
                "diff_summary",
                label_zh="只读差异摘要",
                phases=["diff", "detect"],
                workflow_id="uo-update",
                checker_required=False,
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=["kb-query"],
                output_contract_id="diff-summary-v1",
            ),
            _act(
                "diff_only",
                label_zh="仅差异摘要（跳过完整更新）",
                phases=["detect", "diff"],
                workflow_id="uo-update",
                checker_required=False,
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                capability_ids=["kb-query"],
                output_contract_id="diff-summary-v1",
            ),
        ],
        "agents": [
            {"id": "uo-semantic-resolve", "role": "producer"},
            {"id": "uo-key-resolve", "role": "producer"},
            {"id": "uo-confidence-review", "role": "referee"},
            {"id": "uo-kb-review", "role": "referee"},
            {"id": "deterministic-uo-engine", "role": "deterministic_engine"},
        ],
        "static_obligations": [{"id": "kb_integrity_passed", "label_zh": "完整性通过"}],
        "dynamic_obligation_sources": ["ir/unresolved.yaml", "ir/input_derivable_gaps.yaml"],
        "write_roots": ["uo", "runs", "state", "context"],
        "reset_policy": {
            "reinit_delete": [
                "uo/diff",
                "uo/summary/update_plan.yaml",
            ],
            "reinit_preserve": ["uo"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["detect", "plan", "apply", "resolve", "export", "diff"],
        "gates": [
            "confidence_gate",
            "integrity",
        ],
    },
    "uo-query": {
        "slash": "/uo-query",
        "engine": "uo",
        "entry_state": "route",
        "terminal_ready_states": ["answer"],
        "retry_budget": 3,
        "states": [
            _st("route", "问题路由"),
            _st("lookup", "知识检索"),
            _st("answer", "回答"),
        ],
        "transitions": [
            _tr("route", "lookup"),
            _tr("lookup", "answer"),
            _tr("lookup", "lookup", kind="rework", reason_codes=["LOOKUP_REWORK"]),
        ],
        "phase_gates": {"route": ["kb_ready"]},
        "complete_gates": ["kb_ready"],
        "actions": [
            _act(
                "kb_lookup",
                label_zh="KB 查询",
                phases=["route", "lookup", "answer"],
                workflow_id="uo-query",
                agent_id="uo-query",
                role_id="readonly_analyst",
                gates=["kb_ready"],
                capability_ids=["kb-query", "cbm-navigation", "source-reading"],
                task_prompt_id="uo/kb-lookup",
                output_contract_id="kb-answer-v1",
            ),
        ],
        "agents": [{"id": "uo-query", "role": "readonly_analyst"}],
        "static_obligations": [{"id": "kb_ready", "label_zh": "KB 就绪"}],
        "dynamic_obligation_sources": [],
        "write_roots": ["runs", "context", "memory"],
        "reset_policy": {
            "reinit_delete": [],
            "reinit_preserve": ["uo", "tg", "ce"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["route", "lookup", "answer"],
        "gates": ["kb_ready"],
    },
    "ce-review": {
        "slash": "/ce-review",
        "engine": "ce",
        "entry_state": "context",
        "terminal_ready_states": ["summary"],
        "retry_budget": 3,
        "states": [
            _st("context", "组装上下文"),
            _st("bug", "缺陷审查"),
            _st("functional", "功能审查"),
            _st("summary", "汇总"),
        ],
        "transitions": [
            _tr("context", "bug"),
            _tr("bug", "functional"),
            _tr("functional", "summary"),
        ],
        "phase_gates": {"context": ["kb_ready", "context_pack"]},
        "complete_gates": ["kb_ready", "context_pack"],
        "actions": [
            _act(
                "code_review",
                label_zh="代码审查",
                phases=["context", "bug", "functional", "summary"],
                workflow_id="ce-review",
                agent_id="ce-reviewer",
                role_id="readonly_reviewer",
                capability_ids=["structured-review", "kb-query", "cbm-navigation", "source-reading"],
                task_prompt_id="ce/code-review",
                output_contract_id="code-review-v1",
            ),
        ],
        "agents": [{"id": "ce-reviewer", "role": "readonly_reviewer"}],
        "static_obligations": [{"id": "kb_ready", "label_zh": "KB 就绪"}],
        "dynamic_obligation_sources": [],
        "write_roots": ["ce/review", "runs", "context"],
        "reset_policy": {
            "reinit_delete": ["ce/review"],
            "reinit_preserve": ["uo", "tg"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["context", "bug", "functional", "summary"],
        "gates": ["kb_ready", "context_pack"],
    },
    "tg-init": {
        "slash": "/tg-init",
        "engine": "tg",
        "entry_state": "kb_ready",
        "terminal_ready_states": ["confirm"],
        "retry_budget": 3,
        "states": [
            _st("kb_ready", "KB 检查"),
            _st("contract", "合同构建"),
            _st("bind", "语义绑定"),
            _st("merge", "绑定合并"),
            _st("nest", "中间量闭合"),
            _st("gate", "完整性校验"),
            _st("confirm", "人工确认"),
        ],
        "transitions": [
            _tr("kb_ready", "contract"),
            _tr("contract", "bind"),
            _tr("bind", "merge"),
            _tr("merge", "nest"),
            _tr("nest", "gate"),
            _tr("gate", "confirm"),
            _tr("merge", "bind", kind="rework", reason_codes=["BIND_REWORK"]),
            _tr("gate", "merge", kind="rework", reason_codes=["MERGE_REWORK"]),
            _tr("confirm", "gate", kind="rework", reason_codes=["AUDIT_REWORK"]),
        ],
        "phase_gates": {
            "kb_ready": ["uo_ready", "input_derivable_closed"],
            "bind": ["bind_progress"],
            "merge": ["merge_pass"],
            "nest": ["family_path_obligation"],
            "gate": ["domain_symmetry", "csv_closure", "audit_pass", "family_path_obligation"],
            "confirm": ["init_confirmed", "kb_fingerprint_fresh"],
        },
        "complete_gates": [
            "uo_ready",
            "input_derivable_closed",
            "family_path_obligation",
            "merge_pass",
            "domain_symmetry",
            "csv_closure",
            "audit_pass",
            "init_confirmed",
            "kb_fingerprint_fresh",
        ],
        "actions": [
            _act(
                "kb_check",
                label_zh="校验定稿 KB",
                phases=["kb_ready"],
                workflow_id="tg-init",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                gates=["uo_ready", "input_derivable_closed"],
                capability_ids=["kb-query"],
                output_contract_id="uo-ready-v1",
            ),
            _act(
                "contract_build",
                label_zh="构建合同骨架",
                phases=["contract"],
                workflow_id="tg-init",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                capability_ids=_CAPS_CONTRACT,
                output_contract_id="csv-contract-v1",
            ),
            _act(
                "semantic_bind",
                label_zh="语义绑定",
                phases=["bind"],
                workflow_id="tg-init",
                agent_id="tg-semantic-bind",
                role_id="producer",
                gates=["bind_progress"],
                capability_ids=["kb-query", "semantic-resolution"],
                task_prompt_id="tg/semantic-bind",
                output_contract_id="semantic-bind-v1",
                output_mode="staged",
                staging_contract_id="semantic-bind-patch-v1",
                merge_action_id="semantic_bind",
            ),
            _act(
                "bind_merge",
                label_zh="绑定合并",
                phases=["merge"],
                workflow_id="tg-init",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                gates=["merge_pass"],
                capability_ids=[],
                output_contract_id="bind-merge-v1",
            ),
            _act(
                "mid_nest",
                label_zh="中间量闭合",
                phases=["nest"],
                workflow_id="tg-init",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                capability_ids=["obligation-analysis"],
                output_contract_id="mid-nest-v1",
            ),
            _act(
                "integrity_gate",
                label_zh="完整性校验",
                phases=["gate"],
                workflow_id="tg-init",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                gates=["domain_symmetry", "csv_closure"],
                capability_ids=[],
                output_contract_id="tg-integrity-v1",
            ),
            _act(
                "init_audit",
                label_zh="Init 审计",
                phases=["gate"],
                workflow_id="tg-init",
                agent_id="tg-init-audit",
                role_id="referee",
                referee_required=True,
                gates=["audit_pass"],
                capability_ids=_CAPS_REVIEW,
                task_prompt_id="tg/init-audit",
                output_contract_id="init-audit-v1",
            ),
            _act(
                "human_confirm",
                label_zh="人工确认",
                phases=["confirm"],
                workflow_id="tg-init",
                agent_id=None,
                role_id=None,
                gates=["init_confirmed", "kb_fingerprint_fresh"],
                capability_ids=[],
                task_prompt_id="tg/human-confirm",
                output_contract_id="init-confirmed-v1",
            ),
        ],
        "agents": [
            {"id": "tg-semantic-bind", "role": "producer"},
            {"id": "tg-init-audit", "role": "referee"},
            {"id": "deterministic-tg-engine", "role": "deterministic_engine"},
        ],
        "static_obligations": [
            {"id": "uo_ready", "label_zh": "UO KB 就绪"},
            {"id": "init_confirmed", "label_zh": "Init 已确认"},
        ],
        "dynamic_obligation_sources": [
            "realization/unresolved.yaml",
            "realization/binding_gaps.yaml",
        ],
        "write_roots": ["tg", "runs", "state", "context"],
        "read_only_uo": True,
        "reset_policy": {
            "reinit_delete": ["tg"],
            "reinit_preserve": ["uo"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["kb_ready", "contract", "bind", "merge", "nest", "gate", "confirm"],
        "gates": [
            "uo_ready",
            "kb_fingerprint_fresh",
            "bind_progress",
            "merge_pass",
            "domain_symmetry",
            "csv_closure",
            "audit_pass",
            "init_confirmed",
        ],
    },
    "tg-plan": {
        "slash": "/tg-plan",
        "engine": "tg",
        "entry_state": "scope",
        "terminal_ready_states": ["approve"],
        "retry_budget": 3,
        "states": [
            _st("scope", "规划范围"),
            _st("gate", "前置门禁"),
            _st("build", "生成义务"),
            _st("filter", "过滤裁剪"),
            _st("review", "规划审查"),
            _st("approve", "人工批准"),
        ],
        "transitions": [
            _tr("scope", "gate"),
            _tr("gate", "build"),
            _tr("build", "filter"),
            _tr("filter", "review"),
            _tr("review", "approve"),
            _tr("approve", "review", kind="rework", reason_codes=["PLAN_REWORK"]),
        ],
        "phase_gates": {
            "gate": ["tg_init_confirmed", "kb_fingerprint_fresh"],
            "approve": ["plan_approved"],
        },
        "complete_gates": ["tg_init_confirmed", "plan_approved"],
        "actions": [
            _act(
                "plan_scope",
                label_zh="确定规划范围",
                phases=["scope"],
                workflow_id="tg-plan",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                capability_ids=_CAPS_OBLIGATION,
                output_contract_id="plan-scope-v1",
            ),
            _act(
                "plan_precheck",
                label_zh="规划前置门禁",
                phases=["gate"],
                workflow_id="tg-plan",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                gates=["tg_init_confirmed", "kb_fingerprint_fresh"],
                capability_ids=[],
                output_contract_id="plan-precheck-v1",
            ),
            _act(
                "plan_build",
                label_zh="生成覆盖义务",
                phases=["build", "filter", "review"],
                workflow_id="tg-plan",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                capability_ids=_CAPS_OBLIGATION,
                output_contract_id="plan-build-v1",
            ),
            _act(
                "plan_approve",
                label_zh="批准规划",
                phases=["approve"],
                workflow_id="tg-plan",
                agent_id=None,
                role_id=None,
                gates=["plan_approved"],
                capability_ids=[],
                task_prompt_id="tg/plan-approve",
                output_contract_id="plan-approved-v1",
            ),
        ],
        "agents": [
            {"id": "deterministic-tg-engine", "role": "deterministic_engine"},
        ],
        "static_obligations": [{"id": "plan_approved", "label_zh": "规划已批准"}],
        "dynamic_obligation_sources": ["plan/**/uncovered_obligations.yaml"],
        "write_roots": ["tg", "runs", "state"],
        "read_only_uo": True,
        "reset_policy": {
            "reinit_delete": ["tg"],
            "reinit_preserve": ["uo"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["scope", "gate", "build", "filter", "review", "approve"],
        "gates": ["tg_init_confirmed", "plan_approved"],
    },
    "tg-solve": {
        "slash": "/tg-solve",
        "engine": "tg",
        "entry_state": "gate",
        "terminal_ready_states": ["cover"],
        "retry_budget": 3,
        "states": [
            _st("gate", "求解前置"),
            _st("encode", "编码约束"),
            _st("solve", "Z3 求解"),
            _st("project", "CSV 投影"),
            _st("cover", "覆盖确认"),
        ],
        "transitions": [
            _tr("gate", "encode"),
            _tr("encode", "solve"),
            _tr("solve", "project"),
            _tr("project", "cover"),
            _tr("solve", "encode", kind="rework", reason_codes=["SOLVE_REWORK"]),
        ],
        "phase_gates": {
            "gate": ["plan_approved", "kb_fingerprint_fresh"],
            "cover": ["solve_terminal"],
        },
        "complete_gates": ["plan_approved", "solve_terminal", "domain_symmetry"],
        "actions": [
            _act(
                "solve_precheck",
                label_zh="求解前置校验",
                phases=["gate"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                gates=["plan_approved", "kb_fingerprint_fresh"],
                capability_ids=[],
                output_contract_id="solve-precheck-v1",
            ),
            _act(
                "z3_solve",
                label_zh="求解并投影",
                phases=["encode", "solve", "project"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                capability_ids=["obligation-analysis"],
                output_contract_id="z3-solve-v1",
            ),
            _act(
                "cover_confirm",
                label_zh="覆盖确认",
                phases=["cover"],
                workflow_id="tg-solve",
                agent_id="deterministic-tg-engine",
                role_id="deterministic_engine",
                gates=["solve_terminal"],
                capability_ids=["obligation-analysis"],
                output_contract_id="cover-confirm-v1",
            ),
        ],
        "agents": [
            {"id": "deterministic-tg-engine", "role": "deterministic_engine"},
        ],
        "static_obligations": [{"id": "solve_terminal", "label_zh": "求解终态完成"}],
        "dynamic_obligation_sources": ["solve/**/uncovered_obligations.yaml"],
        "write_roots": ["tg", "runs", "state"],
        "read_only_uo": True,
        "reset_policy": {
            "reinit_delete": ["tg"],
            "reinit_preserve": ["uo"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "phases": ["gate", "encode", "solve", "project", "cover"],
        "gates": ["plan_approved", "solve_terminal", "domain_symmetry"],
    },
    "tk-cover": {
        "slash": "/tk-cover",
        "engine": "uo",
        "entry_state": "prepare",
        "terminal_ready_states": ["certify"],
        "retry_budget": 3,
        "reset_policy": {
            "reinit_delete": ["uo/tk"],
            "reinit_preserve": ["uo/ir", "uo/indexes", "uo/cbm", "uo/derive"],
            "reinit_wipe_runs": "current",
            "continue_scrub": "from_contracts",
        },
        "states": [
            _st("prepare", "环境探测"),
            _st("derive", "派生与地图"),
            _st("close", "缺口闭合"),
            _st("certify", "覆盖门禁"),
        ],
        "transitions": [
            _tr("prepare", "derive"),
            _tr("derive", "close"),
            _tr("close", "certify"),
            _tr("close", "derive", kind="rework", reason_codes=["DERIVE_REWORK"]),
            _tr("certify", "close", kind="rework", reason_codes=["GATE_REWORK"]),
        ],
        "phase_gates": {
            "prepare": ["tk_env"],
            "derive": ["tk_derive", "tk_codemap"],
            "close": ["tk_mine", "tk_recipe"],
            "certify": ["tk_gate"],
        },
        "pipelines": {
            "prepare": ["env_probe"],
            "derive": ["derive_fields", "export_codemap"],
            "close": ["mine_recipe", "apply_recipe"],
            "certify": ["coverage_gate"],
        },
        "complete_gates": ["tk_env", "tk_derive", "tk_codemap", "tk_mine", "tk_recipe", "tk_gate"],
        "meta": {
            "max_recipe_attempts": 3,
            "recovery_by_reason": {
                "DERIVE_REWORK": {"type": "action", "action_id": "derive_fields"},
                "GATE_REWORK": {"type": "action", "action_id": "mine_recipe"},
            },
        },
        "actions": [
            _act(
                "env_probe",
                label_zh="探测 replay 环境",
                phases=["prepare"],
                workflow_id="tk-cover",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["tk_env"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="tk-env-v1",
            ),
            _act(
                "derive_fields",
                label_zh="读取宿主派生",
                phases=["derive"],
                workflow_id="tk-cover",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["tk_derive"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="tk-derive-v1",
            ),
            _act(
                "export_codemap",
                label_zh="导出 HostIR codemap",
                phases=["derive"],
                workflow_id="tk-cover",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["tk_codemap"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="tk-codemap-v1",
            ),
            _act(
                "mine_recipe",
                label_zh="挖掘覆盖 recipe",
                phases=["close"],
                workflow_id="tk-cover",
                agent_id="tk-recipe-miner",
                role_id="producer",
                execution_mode="subagent",
                gates=["tk_mine"],
                capability_ids=_CAPS_RESOLVE,
                task_prompt_id="tk/mine-recipe",
                output_contract_id="tk-recipe-staging-v1",
                output_mode="staged",
                staging_contract_id="tk-recipe-staging-v1",
                merge_action_id="apply_recipe",
                allowed_write_paths=[
                    "runs/{run_id}/actions/mine_recipe/parts/**",
                    "runs/{run_id}/actions/mine_recipe/scratch/**",
                    "runs/{run_id}/actions/mine_recipe/staging.yaml",
                ],
            ),
            _act(
                "apply_recipe",
                label_zh="应用覆盖 recipe",
                phases=["close"],
                workflow_id="tk-cover",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["tk_recipe"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="tk-recipe-v1",
            ),
            _act(
                "coverage_gate",
                label_zh="运行时反例门",
                phases=["certify"],
                workflow_id="tk-cover",
                agent_id="deterministic-uo-engine",
                role_id="deterministic_engine",
                gates=["tk_gate"],
                capability_ids=[],
                task_prompt_id=None,
                output_contract_id="tk-gate-v1",
            ),
        ],
        "agents": [
            {"id": "deterministic-uo-engine", "role": "deterministic_engine"},
            {"id": "tk-recipe-miner", "role": "producer"},
        ],
        "write_roots": ["uo", "runs", "state"],
        "phases": ["prepare", "derive", "close", "certify"],
        "gates": ["tk_env", "tk_derive", "tk_codemap", "tk_mine", "tk_recipe", "tk_gate"],
    },
    "code-edit": {"slash": None, "engine": "ext", "phases": [], "gates": [], "states": [], "reserved": True},
    "git-ops": {"slash": None, "engine": "ext", "phases": [], "gates": [], "states": [], "reserved": True},
    "build": {"slash": None, "engine": "ext", "phases": [], "gates": [], "states": [], "reserved": True},
    "test-run": {"slash": None, "engine": "ext", "phases": [], "gates": [], "states": [], "reserved": True},
    "perf-analyze": {"slash": None, "engine": "ext", "phases": [], "gates": [], "states": [], "reserved": True},
}
